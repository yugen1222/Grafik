...
const managerViews = {
  ...
  "service1": { title: "Обслуживание · 1 смена", shift: "1 смена", categories: ["Менеджер", "Бариста", "Официант"], serviceOnly: true },
  "branch2": { title: "Филиал · 2 смена", shift: "2 смена", categories: ["Менеджер", "Кассир", "Продавец", "Морозильщик", "Техничка"] },
  "service2": { title: "Обслуживание · 2 смена", shift: "2 смена", categories: ["Менеджер", "Бариста", "Официант"], serviceOnly: true },
...
function render(options = {}){
  ...
  const pageTitles = { managerBoard:"Рабочий график", dashboard:"Помощник", schedule:"График недели", attendance:"Посещаемость", dayoffs:"Выходные", employees:"Сотрудники", staffing:"Штатка", history:"История", skills:"Навыки" };
  ...
  renderWeekSelect();
  renderManagerBoard(); renderDashboard(); renderSchedule(); renderDayoffs(); renderAttendance(); renderEmployees(); renderSkills(); renderStaffing(); renderHistory();
  isRendering = false;
  if(shouldSave) saveState();
}
...
function managerRowsForCategory(category, view){
  return state.employees.filter(emp => {
    ...
    const hasWeekAssignment = getWeekDays(state.currentWeek).map(dateKey).some(day =>
      assignmentForManagerCell(day, emp.id, category, view.shift)
    );
...
function managerCell(day, emp, category, view){
  ...
  if(!item){
    return `<td class="managerCell emptyCell" onclick="openEmployeeDrawer('${day}','${emp.id}','${category}','${emp.defaultShift}')">+</td>`;
  }
