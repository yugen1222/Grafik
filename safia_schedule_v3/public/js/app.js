...
function copiedScheduleForNewWeekV171(startKey, copyFromKey){
  const days = getWeekDays(startKey).map(dateKey);
  const oldDays = getWeekDays(copyFromKey).map(dateKey);
  const schedule = {};
  const sourceWeek = state.weeks?.[copyFromKey];
  const activeEmployees = state.employees.filter(e=>e.active);
  const activeIds = new Set(activeEmployees.map(e=>e.id));

  // Сотрудник считается уже существовавшим в прошлой неделе, если он хотя бы
  // один раз был там назначен или имел явный выходной. Только такие сотрудники
  // автоматически попадают в новую неделю. Новые сотрудники по-прежнему остаются
  // «Не назначен», пока менеджер не поставит их вручную.
  const establishedIds = new Set();
  oldDays.forEach(oldDay => {
    (sourceWeek?.schedule?.[oldDay] || []).forEach(item => {
      if(item?.employeeId) establishedIds.add(item.employeeId);
    });
  });

  days.forEach((day, index) => {
    const oldItems = sourceWeek?.schedule?.[oldDays[index]] || [];
    const result = [];

    activeEmployees.forEach(emp => {
      const employeeItems = oldItems.filter(item => item.employeeId === emp.id);
      const workItems = employeeItems.filter(item => item.status !== "off" && item.category && item.shift);

      if(workItems.length){
        // Рабочее назначение копируем как основу новой недели.
        workItems.forEach(item => result.push({
          employeeId: emp.id,
          category: item.category,
          shift: item.shift,
          status: "work"
        }));
      } else if(establishedIds.has(emp.id)){
        // КЛЮЧЕВОЕ ПРАВИЛО v18.1:
        // отсутствие назначения / выходной в прошлой неделе НЕ переносится.
        // Сотрудник возвращается в свою обычную должность и постоянную смену.
        result.push({
          employeeId: emp.id,
    ...
    schedule[day] = result.filter(item => activeIds.has(item.employeeId));
  });
...
function createWeek(startKey, copyFromKey=null){
  ...
  if(copyFromKey && state.weeks[copyFromKey]){
    schedule = copiedScheduleForNewWeekV171(startKey, copyFromKey);
  } else {
    days.forEach(day => {
      schedule[day] = [];
  ...
  state.weeks[startKey] = {
    ...
    cleanWeekVersion: "18.1"
  };
}

function repairUntouchedCopiedWeekV171(weekKey, copyFromKey){
  const week = state.weeks?.[weekKey];
  const source = state.weeks?.[copyFromKey];
  if(!week || !source) return false;
  if(week.cleanWeekVersion === "18.1") return false;

  const history = Array.isArray(week.history) ? week.history : [];
  const attendanceCount = Object.keys(week.attendance || {}).length;
  const untouched = history.length <= 1 && attendanceCount === 0;
  if(!untouched) return false;

  // Неделя была создана старой версией, но менеджер ещё не начал её менять.
  // Пересобираем только график. Штатку и пожелания сотрудников сохраняем.
  week.schedule = copiedScheduleForNewWeekV171(weekKey, copyFromKey);
  week.cleanWeekVersion = "18.1";
  week.history = history.length ? history : [`Создана неделя ${weekLabel(weekKey)}`];
  return true;
}


// v18.1 — исправляет уже созданную будущую неделю, в которой старые выходные
// превратились в пустые клетки «Не назначен». Работает только если сотрудники
// ещё не отправляли выбор выходных на эту неделю.
function repairInheritedDayOffGapsV181(weekKey, copyFromKey){
  const week=state.weeks?.[weekKey], source=state.weeks?.[copyFromKey];
  if(!week||!source) return false;
  if(week.cleanWeekVersion === "18.1") return false;

  const prefs=week.employeePreferences||{};
  const hasSubmitted=Object.values(prefs).some(p=>Boolean(p?.submittedAt));
  if(hasSubmitted) return false;

  const targetDays=getWeekDays(weekKey).map(dateKey);
  const sourceDays=getWeekDays(copyFromKey).map(dateKey);
  const establishedIds=new Set();
  sourceDays.forEach(day=>(source.schedule?.[day]||[]).forEach(item=>{ if(item?.employeeId) establishedIds.add(item.employeeId); }));

  let repaired=0;
  targetDays.forEach((day,index)=>{
    week.schedule[day]=Array.isArray(week.schedule?.[day])?week.schedule[day]:[];
    const sourceItems=source.schedule?.[sourceDays[index]]||[];
    state.employees.filter(e=>e.active&&establishedIds.has(e.id)).forEach(emp=>{
      const targetEmpItems=week.schedule[day].filter(i=>i.employeeId===emp.id);
      const targetHasWork=targetEmpItems.some(isWorkItem);
      const explicitTargetOff=targetEmpItems.some(i=>i.status==='off') || (prefs?.[emp.id]?.offDays||[]).includes(day);
      if(targetHasWork||explicitTargetOff) return;

      const sourceEmpItems=sourceItems.filter(i=>i.employeeId===emp.id);
      const sourceHadNoWork=!sourceEmpItems.some(isWorkItem);
      const sourceHadOff=sourceEmpItems.some(i=>i.status==='off');
      if(sourceHadNoWork||sourceHadOff){
        week.schedule[day].push({employeeId:emp.id,category:emp.position,shift:emp.defaultShift,status:'work',source:'cleanWeekV181'});
        repaired++;
      }
    });
  });
  ...
  if(repaired){
    ...
    week.history.push(`v18.1: убраны повторившиеся выходные прошлой недели (${repaired})`);
  }
  return repaired>0;
}
...
function createNextWeek(){
  const sourceKey = state.currentWeek;
  const s=new Date(sourceKey); s.setDate(s.getDate()+7); const key=dateKey(s);
  if(!state.weeks[key]){
    createWeek(key,sourceKey);
  } else {
    repairInheritedDayOffGapsV181(key,sourceKey) || repairUntouchedCopiedWeekV171(key,sourceKey);
  }
...
function weeklyEmployeeMetrics(empId){
  ensureAttendance();
  const days=getWeekDays(state.currentWeek).map(dateKey);
  let planned=0,came=0,absent=0,early=0,replaced=0,replacements=0,fulls=0,violations=0,unmarked=0;
  days.forEach(day=>{
    ...
    items.forEach(i=>{
      const a=state.weeks[state.currentWeek].attendance[attendanceKey(day,empId,i.shift,i.category)];
      if(!a){ unmarked++; }
      else if(a.status==="Вышел") came++;
      else if(a.status==="Не вышел") absent++;
      else if(a.status==="Ушёл раньше") early++;
      if(a?.replacementId) replaced++;
      if(i.replacement) replacements++;
      if(i.violation) violations++;
    });
  });
  const marked=came+absent+early;
  const present=came+early;
  // Неотмеченные смены не считаем как невыход: процент строится только по подтвержденным отметкам.
  const attendancePercent=marked?Math.round((present/marked)*100):null;
  const score=Math.max(0,Math.min(100,Math.round(100-(absent*20)-(early*10)-(violations*15)+(replacements*3))));
  return {planned,came,absent,early,replaced,replacements,fulls,violations,score,marked,unmarked,present,attendancePercent};
}
function employeeRatingNote(empId){
  const m=weeklyEmployeeMetrics(empId);
  const notes=[];
  if(m.replaced>=2) notes.push(`⚠ Часто требуется замена (${m.replaced})`);
  if(m.absent>=2) notes.push(`⚠ Частые невыходы (${m.absent})`);
  if(m.fulls>=MAX_FULLS_IN_7_DAYS) notes.push(`⚡ Высокая нагрузка: ${m.fulls} FULL`);
  if(m.early>=2) notes.push(`🟡 Часто уходит раньше (${m.early})`);
  return notes;
}
...
function renderAttendance(){
  ...
  const report=state.employees.filter(e=>e.active).map(e=>{const m=weeklyEmployeeMetrics(e.id);const r=recommendationFor(e.id);const notes=employeeRatingNote(e.id);return `<tr><td><b>${shortName(e.name)}</b>${notes.length?`<br><small class="employeeRatingNoteV162">${notes.join("<br>")}</small>`:""}</td><td>${m.planned}</td><td><b>${m.attendancePercent===null?"—":m.attendancePercent+"%"}</b><br><small>отмечено ${m.marked}/${m.planned}</small></td><td>${m.came}</td><td>${m.absent}</td><td>${m.early}</td><td><b>${m.replaced}</b></td><td><b>${m.replacements}</b></td><td>${m.fulls}</td><td><b>${m.score}</b></td><td>${r.days?`Рекомендуется ${r.days} выходных<br><small>${r.reasons.join(", ")}</small>`:"—"}</td></tr>`}).join("");
  ...
  <div class="card admin-only"><h3>Рейтинг соблюдения графика за неделю</h3><p class="muted">«Заменяли» — сколько раз вместо сотрудника выходил другой. «Сам заменял» — сколько раз сотрудник сам выходил вместо коллеги.</p><div class="tableScroll"><table class="table"><thead><tr><th>Сотрудник</th><th>План</th><th>Посещаемость</th><th>Вышел</th><th>Не вышел</th><th>Ушёл раньше</th><th>Заменяли</th><th>Сам заменял</th><th>FULL</th><th>Рейтинг</th><th>Рекомендация</th></tr></thead><tbody>${report}</tbody></table></div></div>`;
}
...
// ================= SAFIA SCHEDULE V14.3 STABLE FIX: ЗАЩИТА ДАННЫХ И АВТОВОССТАНОВЛЕНИЕ =================
...
const SAFIA_RECOVERY_EMPLOYEES_V143 = [{"active":true,"defaultShift":"16:00-23:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e1","name":"ABDIYEVA SHAHINABONU SUNNATULLO QIZI","position":"Продавец","serviceScore":50,"skills":["Продавец"],"universal":false},{"active":true,"defaultShift":"23:00-08:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e2","name":"ABDUKARIMOV UMIDJON ILXOM O'G'LI","position":"Продавец","serviceScore":50,"skills":["Продавец"],"universal":false},{"active":true,"defaultShift":"16:00-23:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e3","name":"ABDULLAYEVA SEVINCHOY AHMADJON QIZI","position":"Официант","serviceScore":50,"skills":["Официант"],"universal":false},{"active":true,"defaultShift":"16:00-23:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e5","name":"ABDUSALIMOV FAXRIDDIN IMOMALI O'G'LI","position":"Официант","serviceScore":50,"skills":["Официант"],"universal":false},{"active":true,"defaultShift":"20:00-08:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e6","name":"AGZAMOV RAMZIDDIN SHUHRAT O'G'LI","position":"Кассир","skills":["Кассир"],"universal":false},{"active":true,"defaultShift":"08:00-16:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e7","name":"AKBAROVA GULCHEXRA MAXMUDJONOVNA","position":"Техничка","skills":["Техничка"],"universal":false},{"active":true,"defaultShift":"20:00-08:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e8","name":"ANORBOYEV ABDULATIF NURIDDIN O'G'LI","position":"Морозильщик","skills":["Морозильщик"],"universal":false},{"active":true,"defaultShift":"08:00-16:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e9","name":"AXMEDJANOVA BIBIXOJAR OYBEK QIZI","position":"Официант","serviceScore":50,"skills":["Официант"],"universal":false},{"active":true,"defaultShift":"23:00-08:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e10","managerPanel":"shift3","name":"AXMEDOVA NIGORAXON ULUGBEK QIZI","position":"Менеджер","skills":["Менеджер"],"universal":false},{"active":true,"defaultShift":"08:00-16:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e11","name":"BEKNAZAROVA MAHLIYO G'ULOMJON QIZI","position":"Продавец","serviceScore":50,"skills":["Продавец"],"universal":false},{"active":true,"defaultShift":"08:00-16:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e12","name":"DADABOYEVA AZIZAXON DILSHOD QIZI","position":"Официант","serviceScore":50,"skills":["Официант"],"universal":false},{"active":true,"defaultShift":"16:00-23:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e13","name":"ERGASHEVA DILAFRUZ MURODJON QIZI","position":"Продавец","serviceScore":50,"skills":["Продавец"],"universal":false},{"active":true,"defaultShift":"16:00-23:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e14","name":"ERGASHOVA AZIZA XOLIYOR QIZI","position":"Техничка","skills":["Техничка"],"universal":false},{"active":true,"defaultShift":"20:00-08:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e15","name":"FOZILOVA KOMILA KAMOLITDINOVNA","position":"Техничка","skills":["Техничка"],"universal":false},{"active":true,"defaultShift":"16:00-23:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e16","name":"G'OFUROV ABDULATIF AKMAL O'G'LI","position":"Бариста","skills":["Бариста"],"universal":false},{"active":true,"defaultShift":"16:00-23:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e17","name":"GAFUROVA SURAYYO ABDUNABIYEVNA","position":"Техничка","skills":["Техничка"],"universal":false},{"active":true,"defaultShift":"08:00-16:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e18","managerPanel":"branch1","name":"GAPPAROV MUHAMMADRIZO SHARIF O'G'LI","position":"Менеджер","skills":["Менеджер официантской зоны","Менеджер"],"universal":false},{"active":true,"defaultShift":"16:00-00:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e19","managerPanel":"branch2","name":"IBODULLAYEV YODGOR OZOD O'G'LI","position":"Менеджер","skills":["Менеджер официантской зоны","Менеджер"],"universal":false},{"active":true,"defaultShift":"18:00-02:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e20","name":"IKROMOV ZAFARBEK MURODBEK O'G'LI","position":"Официант","serviceScore":50,"skills":["Официант"],"universal":false},{"active":true,"defaultShift":"18:00-02:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e21","name":"ISAXOVA VENERA ANSATBAY QIZI","position":"Официант","serviceScore":50,"skills":["Официант"],"universal":false},{"active":true,"defaultShift":"16:00-23:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e22","name":"ISMATILLAYEVA MALIKA MARDON QIZI","position":"Бариста","skills":["Бариста"],"universal":false},{"active":true,"defaultShift":"16:00-23:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e23","name":"ISMOILOVA KRISTINA TAXIROVNA","position":"Техничка","skills":["Техничка"],"universal":false},{"active":true,"defaultShift":"08:00-20:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e24","name":"JALILOV ABDURAHMON RAVSHAN O'G'LI","position":"Морозильщик","skills":["Морозильщик"],"universal":false},{"active":true,"defaultShift":"08:00-16:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e25","name":"JANGIROVA IRODAXON BAHODIR QIZI","position":"Официант","serviceScore":50,"skills":["Официант"],"universal":false},{"active":true,"defaultShift":"08:00-20:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e26","name":"JO'RAYEVA HUSNORA BOZAROVNA","position":"Техничка","skills":["Техничка"],"universal":false},{"active":true,"defaultShift":"08:00-16:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e27","name":"KADIROVA MAPURA TAXIROVNA","position":"Продавец","serviceScore":50,"skills":["Продавец"],"universal":false},{"active":true,"defaultShift":"23:00-08:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e28","name":"KARIMJONOV AVAZBEK MURODJON O'G'LI","position":"Бариста","skills":["Бариста"],"universal":false},{"active":true,"defaultShift":"09:00-18:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e29","managerPanel":"branch1","name":"KRIVOROTOVA SVETLANA VITALEVNA","position":"Менеджер","skills":["Менеджер официантской зоны","Менеджер"],"universal":false},{"active":true,"defaultShift":"08:00-16:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e30","name":"KUSHSHAYEV ISLOMBEK KAXRAMON O'G'LI","position":"Официант","serviceScore":50,"skills":["Официант"],"universal":false},{"active":true,"defaultShift":"16:00-23:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e31","name":"MANSUROV DAVRONBEK AZIZBEKOVICH","position":"Официант","serviceScore":50,"skills":["Официант"],"universal":false},{"active":true,"defaultShift":"16:00-23:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e32","name":"MAXMUDOVA E'ZOZAXON ABDUSAMAD QIZI","position":"Продавец","serviceScore":50,"skills":["Продавец"],"universal":false},{"active":true,"defaultShift":"08:00-16:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e33","name":"MUXAMEDOVA SHAXNOZA KAMILOVNA","position":"Техничка","skills":["Техничка"],"universal":false},{"active":true,"defaultShift":"16:00-23:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e34","name":"NASRULLOVA MALIKA BAXTIYOROVNA","position":"Продавец","serviceScore":50,"skills":["Продавец"],"universal":false},{"active":true,"defaultShift":"08:00-16:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e35","name":"NISHONBOYEVA NOZIMA JAMSHID QIZI","position":"Бариста","skills":["Бариста"],"universal":false},{"active":true,"defaultShift":"16:00-23:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e36","name":"NIZAMATDINOV BEKZOD KOSIM -TALGAT O'G'LI","position":"Бариста","skills":["Бариста"],"universal":false},{"active":true,"defaultShift":"18:00-02:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e37","name":"OLIMBOYEV AFZALBEK ALISHER O'G'LI","position":"Официант","serviceScore":50,"skills":["Официант"],"universal":false},{"active":true,"defaultShift":"08:00-16:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e39","name":"PARDABOYEVA RUSHANA MIRZOXIDOVNA","position":"Официант","serviceScore":50,"skills":["Официант"],"universal":false},{"active":true,"defaultShift":"16:00-23:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e40","name":"PATXITDINOV RASULJON TO'YCHI O'G'LI","position":"Официант","serviceScore":50,"skills":["Официант"],"universal":false},{"active":true,"defaultShift":"16:00-23:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e41","managerPanel":"branch2","name":"RAHMATOVA E'ZOZA AKBAR QIZI","position":"Менеджер","skills":["Менеджер"],"universal":false},{"active":true,"defaultShift":"16:00-23:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e42","name":"RAIMBOYEV G'IYOSJON ERKINBOY O'G'LI","position":"Официант","serviceScore":50,"skills":["Официант"],"universal":false},{"active":true,"defaultShift":"16:00-23:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e43","name":"RASULOVA RUXSHONA RUSTAM QIZI","position":"Официант","serviceScore":50,"skills":["Официант"],"universal":false},{"active":true,"defaultShift":"08:00-20:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e44","name":"RAXMONBERDIYEVA ZUHRA RAXIMBERDI QIZI","position":"Техничка","skills":["Техничка"],"universal":false},{"active":true,"defaultShift":"16:00-23:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e47","name":"SALYAMOVA ZAVXARON FURQAT QIZI","position":"Продавец","serviceScore":50,"skills":["Продавец"],"universal":false},{"active":true,"defaultShift":"16:00-23:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e48","name":"SAYDULLAYEV AZIZBEK DILSHODJON O'G'LI","position":"Официант","serviceScore":50,"skills":["Официант"],"universal":false},{"active":true,"defaultShift":"16:00-23:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e49","name":"SHAMSHIDDINOVA MUXLISA DILSHOD QIZI","position":"Продавец","serviceScore":50,"skills":["Продавец"],"universal":false},{"active":true,"defaultShift":"08:00-16:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e50","name":"TOG'AYBEKOVA NIGORA QUANISH QIZI","position":"Бариста","skills":["Бариста"],"universal":false},{"active":true,"defaultShift":"23:00-08:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e51","name":"TOIROV HASANBOY OTABEK O'G'LI","position":"Официант","serviceScore":50,"skills":["Официант"],"universal":false},{"active":true,"defaultShift":"08:00-16:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e52","name":"TOJIBOYEVA MARJONA ILXOMBOY QIZI","position":"Официант","serviceScore":50,"skills":["Официант"],"universal":false},{"active":true,"defaultShift":"20:00-08:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e53","name":"TURSUNALIYEV OZODBEK ALISHER O'G'LI","position":"Морозильщик","skills":["Морозильщик"],"universal":false},{"active":true,"defaultShift":"08:00-16:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e54","name":"UMARALIYEV SHAHZOD SHERZOD O'GLI","position":"Официант","serviceScore":50,"skills":["Официант"],"universal":false},{"active":true,"defaultShift":"08:00-16:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e55","name":"UMIROVA GULASAL ABDIXALIL QIZI","position":"Официант","serviceScore":50,"skills":["Официант"],"universal":false},{"active":true,"defaultShift":"08:00-16:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e56","managerPanel":"branch1","name":"USMANOVA GULNOZA AZIZ QIZI","position":"Менеджер","skills":["Менеджер"],"universal":false},{"active":true,"defaultShift":"08:00-16:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e57","name":"USMONOV FARRUX USMON O'G'LI","position":"Официант","serviceScore":50,"skills":["Официант"],"universal":false},{"active":true,"defaultShift":"08:00-16:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e58","name":"USMONQULOVA GULCHEHRA KARIMOVNA","position":"Продавец","serviceScore":50,"skills":["Продавец"],"universal":false},{"active":true,"defaultShift":"08:00-16:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e59","name":"VOHIDOVA NASIBA ALIYEVNA","position":"Техничка","skills":["Техничка"],"universal":false},{"active":true,"defaultShift":"08:00-16:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e60","name":"XASHIMOVA SEVARA TAXIROVNA","position":"Продавец","serviceScore":50,"skills":["Продавец"],"universal":false},{"active":true,"defaultShift":"16:00-23:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e61","name":"ZARIPOVA MAFTUNA ZAFAR QIZI","position":"Продавец","serviceScore":50,"skills":["Продавец"],"universal":false},{"active":true,"defaultShift":"08:00-16:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e62","name":"ZOKIRJONOVA ZARINA G'AYRAT QIZI","position":"Бариста","skills":["Бариста"],"universal":false},{"active":true,"defaultShift":"08:00-16:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e63","name":"ZOKIROV ABDULVORIS ABDUQAHHOR O'G'LI","position":"Кассир","skills":["Кассир"],"universal":false},{"active":true,"defaultShift":"08:00-16:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e1784897346701","name":"MIRZAYEV MUROD","position":"Официант","serviceScore":50,"skills":["Официант"],"universal":false},{"active":true,"defaultShift":"16:00-23:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e1783785851071","name":"G'AYRATOV MUXAMMADOLIM RAVSHAN O'G'LI","position":"Официант","serviceScore":50,"skills":["Официант"],"universal":false},{"active":true,"defaultShift":"08:00-16:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e1783785860212","name":"JABKO VIKTORIYA KONSTANTINOVNA","position":"Официант","serviceScore":50,"skills":["Официант"],"universal":false},{"active":true,"defaultShift":"08:00-16:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e1783785869016","name":"XIKMATOV SAMIRJON SA'DULLAYEVICH","position":"Продавец","serviceScore":50,"skills":["Продавец"],"universal":false},{"active":true,"defaultShift":"08:00-16:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e1783785876966","name":"MARDONQULOVA ZAHRO BOHODIR QIZI","position":"Продавец","serviceScore":50,"skills":["Продавец"],"universal":false},{"active":true,"defaultShift":"08:00-16:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e1785584289198","name":"Mamatkulova Gulnoza","position":"Техничка","skills":["Техничка"],"universal":false},{"active":true,"defaultShift":"16:00-23:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e1785584541352","name":"Najmiddinova Hilolahon","position":"Техничка","skills":["Техничка"],"universal":false},{"active":true,"defaultShift":"08:00-16:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e1785585680915","name":"Namozboyev Nurislom","position":"Официант","serviceScore":50,"skills":["Официант"],"universal":false},{"active":true,"defaultShift":"08:00-20:00","fullBlockAddedBy":"","fullBlockEnd":"","fullBlockReason":"","fullBlockStart":"","fullLimitBlocked":false,"id":"e38","name":"ORIPOVA SARVINOZ DILMUROD QIZI","position":"Кассир","skills":["Кассир"],"universal":false}];
function startFirebaseSyncV14(){
  ...
  cloudLoadedV13=false;
  cloudWriteLockedV14=true;
  showBootV13('Загружаем сотрудников и график…');
  setSyncStatusV112('Загрузка базы…','saving');

  const databaseRef=window.db.ref(window.SAFIA_PATH);
  const stateRef=databaseRef.child('state');
  let firstSnapshot=true;

  stateRef.on('value', async snapshot=>{
    try{
      let rawState=snapshot.val();
      if(!rawState || typeof rawState!=='object') rawState={};

      // Аварийное восстановление касается ТОЛЬКО списка сотрудников.
      // Недели, штатка, история и посещаемость не перезаписываются.
      const cloudEmployees=Array.isArray(rawState.employees) ? rawState.employees : [];
      const activeCloudEmployees=cloudEmployees.filter(e=>e && e.active!==false);
      let employeesRecovered=false;
      if(activeCloudEmployees.length < 10){
        rawState.employees=cloneJson(SAFIA_RECOVERY_EMPLOYEES_V143);
        employeesRecovered=true;
      }

      // Не блокируем показ сотрудников из-за повреждённой одной недели.
      rawState.weeks=(rawState.weeks && typeof rawState.weeks==='object' && !Array.isArray(rawState.weeks)) ? rawState.weeks : {};
      rawState.deletedEmployeeIds=(rawState.deletedEmployeeIds && typeof rawState.deletedEmployeeIds==='object') ? rawState.deletedEmployeeIds : {};

      isRemoteUpdate=true;
      const ui=loadUiSettingsV13();
      let remote=migrateState(cloneJson(rawState));
      remote.deletedEmployeeIds=mergeDeletionRegistriesV13(remote,remote);
      remote=applyDeletionRegistryV13(remote);
      remote.role=ui.role||'admin';

      const weekKeys=Object.keys(remote.weeks||{}).sort();
      remote.currentWeek=(ui.currentWeek && remote.weeks?.[ui.currentWeek])
        ? ui.currentWeek
        : (remote.currentWeek && remote.weeks?.[remote.currentWeek]
          ? remote.currentWeek
          : (weekKeys.pop() || dateKey(weekStart(new Date()))));
      delete remote.__cloudPlaceholder;
      ...
      if(employeesRecovered){
        // Сохраняем только employees, чтобы не затронуть остальную базу.
        await stateRef.child('employees').set(cloneJson(state.employees));
        await databaseRef.update({
          updatedAt:Date.now(),
          updatedBy:`v14.3-employee-recovery:${window.CLIENT_ID||'unknown'}`,
          schemaVersion:14.3
        });
        cloudBaselineV13=cloudStateV13(state);
        setSyncStatusV112(`Сотрудники восстановлены: ${state.employees.length}`,'ok');
        toast(`Восстановлено сотрудников: ${state.employees.length}`);
      }else{
        setSyncStatusV112(`Онлайн · сотрудников ${state.employees.filter(e=>e.active!==false).length}`,'ok');
      }

      if(!firstSnapshot) toast('Данные обновились с другого устройства');
      firstSnapshot=false;
    }catch(err){
      isRemoteUpdate=false;
      console.error('V14.3 employee recovery load error',err);
      // Даже при ошибке показываем аварийный список сотрудников локально.
      try{
        state.employees=cloneJson(SAFIA_RECOVERY_EMPLOYEES_V143);
        ...
        hideBootV13();
        render({save:false});
        setSyncStatusV112(`Аварийный режим · показано ${state.employees.length} сотрудников`,'error');
      }catch(inner){
        showFatalCloudV14(`Не удалось показать сотрудников: ${inner.message||inner}`);
      }
    }
  },error=>{
    ...
    try{
      ...
      hideBootV13();
      render({save:false});
      setSyncStatusV112(`Нет связи · показано ${state.employees.length} сотрудников из резервной копии`,'error');
    }catch(inner){
      showFatalCloudV14('Не удалось прочитать Firebase и открыть резервный список сотрудников.');
    }
  });
}
...
flushStateV13=async function flushStateV14_1(){
  ...
  setSyncStatusV112('Сохраняется…','saving');
  try{
    // Загружаем только state, а не всю ветку вместе с тяжёлыми backups.
    const beforeSnap=await stateRef.once('value');
    const beforeState=beforeSnap.val();
    const beforeValidation=validateCloudStateV14(beforeState);
    if(!beforeValidation.ok) throw new Error(`Текущий state не прошёл проверку: ${beforeValidation.reason}`);

    await saveFirebaseBackupV13(databaseRef,{state:beforeState,updatedAt:Date.now(),updatedBy:'before-save'});

    const result=await stateRef.transaction(currentState=>{
      if(!validateCloudStateV14(currentState).ok) return;
      let merged=cloneJson(currentState);
      Object.entries(diff).forEach(([path,value])=>setPathV13(merged,path,value));
      merged.deletedEmployeeIds=mergeDeletionRegistriesV13(merged,desired);
      merged=applyDeletionRegistryV13(merged);
      if(!validateCloudStateV14(merged).ok) return;
      merged.updatedAt=Date.now();
      return merged;
    });

    if(!result.committed) throw new Error('Firebase transaction cancelled by protection');
    const savedState=result.snapshot.val()||desired;
    cloudBaselineV13=cloudStateV13(savedState);
    await databaseRef.update({
      updatedAt:Date.now(),
      updatedBy:window.CLIENT_ID||'unknown',
      schemaVersion:14.1
    });
    ...
    pendingSaveV13=true;
    setSyncStatusV112('Ошибка сохранения · повторяем','error');
    setTimeout(()=>{ if(!cloudWriteLockedV14) flushStateV13(); },2500);
  }finally{
    activeWriteV13=false;
...
// ================= SAFIA SCHEDULE V16: УБИРАЕМ ЧЕЛОВЕЧЕСКИЙ ФАКТОР =================
// Админ-пароль по умолчанию: Safia2026!
// В коде хранится только SHA-256 хэш, а не сам пароль.
const ADMIN_PASSWORD_HASH_V16 = "7c823b1eb1b546843543456ec4286ef6ee48626b2b5e0314a7e23148acdd5e32";
let planningWeekBeforeAttendanceV16 = null;
let accessModeV16 = null;

async function sha256V16(text){
  const data = new TextEncoder().encode(String(text || ""));
  const digest = await crypto.subtle.digest("SHA-256", data);
  return Array.from(new Uint8Array(digest)).map(b=>b.toString(16).padStart(2,"0")).join("");
}

function setAccessRoleV16(role){
  accessModeV16 = role;
  state.role = role;
  const roleSelect = document.getElementById("roleSelect");
  if(roleSelect) roleSelect.value = role;
  currentPage = "managerBoard";
  if(role === "manager") state.currentManagerId = null;
  saveUiSettingsV13();
  document.getElementById("entryGateV16")?.classList.add("hidden");
  render({save:false});
}

function enterManagerV16(){
  hideAdminLoginV16();
  setAccessRoleV16("manager");
  setTimeout(()=>document.getElementById("managerIdentitySelect")?.focus(),80);
  toast("Выберите себя в панели менеджера");
}

function showAdminLoginV16(){
  document.getElementById("adminLoginBoxV16")?.classList.add("show");
  const input=document.getElementById("adminPasswordV16");
  if(input){ input.value=""; setTimeout(()=>input.focus(),20); }
  const err=document.getElementById("adminLoginErrorV16"); if(err) err.textContent="";
}
function hideAdminLoginV16(){ document.getElementById("adminLoginBoxV16")?.classList.remove("show"); }

async function enterAdminV16(){
  const input=document.getElementById("adminPasswordV16");
  const error=document.getElementById("adminLoginErrorV16");
  const hash=await sha256V16(input?.value || "");
  if(hash !== ADMIN_PASSWORD_HASH_V16){
    if(error) error.textContent="Неверный пароль";
    if(input){ input.value=""; input.focus(); }
    return;
  }
  hideAdminLoginV16();
  setAccessRoleV16("admin");
  toast("Администратор: полный доступ");
}

function logoutAccessV16(){
  accessModeV16=null;
  state.currentManagerId=null;
  const gate=document.getElementById("entryGateV16");
  gate?.classList.remove("hidden");
  hideAdminLoginV16();
}

// Никто не может переключить скрытый roleSelect руками из интерфейса.
const hiddenRoleSelectV16=document.getElementById("roleSelect");
if(hiddenRoleSelectV16){
  hiddenRoleSelectV16.onchange=()=>{
    hiddenRoleSelectV16.value=accessModeV16 || "manager";
    state.role=hiddenRoleSelectV16.value;
  };
}

function actualWeekKeyV16(){ return dateKey(weekStart(new Date())); }
function switchToTodayAttendanceWeekV16(){
  const todayWeek=actualWeekKeyV16();
  if(state.currentWeek !== todayWeek){
    planningWeekBeforeAttendanceV16 = state.currentWeek;
    if(state.weeks?.[todayWeek]) state.currentWeek=todayWeek;
  }
  attendanceSelectedDay=dateKey(new Date());
}
function restorePlanningWeekV16(){
  if(planningWeekBeforeAttendanceV16 && state.weeks?.[planningWeekBeforeAttendanceV16]){
    state.currentWeek=planningWeekBeforeAttendanceV16;
  }
  planningWeekBeforeAttendanceV16=null;
}
function navigateV16(page){
  if(currentPage === "attendance" && page !== "attendance") restorePlanningWeekV16();
  if(page === "attendance") switchToTodayAttendanceWeekV16();
  currentPage=page;
  render({save:false});
}

document.querySelectorAll(".nav").forEach(nav=>{
  nav.onclick=()=>navigateV16(nav.dataset.page);
});

// Менеджеру посещаемость всегда открывается на сегодняшнем дне.
const renderAttendanceBeforeV16=renderAttendance;
renderAttendance=function renderAttendanceV16(){
  if(state.role==="manager") attendanceSelectedDay=dateKey(new Date());
  renderAttendanceBeforeV16();
  const target=document.getElementById("attendance");
  if(!target) return;
  const header=target.querySelector(".attendanceHeaderV113");
  if(header && state.role==="manager"){
    header.classList.add("attendanceLockedV16");
    const select=header.querySelector("select");
    if(select){
      select.value=dateKey(new Date());
      select.disabled=true;
    }
    header.insertAdjacentHTML("beforeend",`<span class="todayAttendanceV16">📍 Сегодня: ${dateKey(new Date())}</span>`);
  }
};

// Даже если менеджер составляет следующую неделю, при входе в посещаемость
// автоматически используется реальная неделя сегодняшнего дня.
const renderActivePageBeforeV16=renderActivePageV112;
renderActivePageV112=function renderActivePageV16(){
  if(currentPage==="attendance" && state.role==="manager") switchToTodayAttendanceWeekV16();
  return renderActivePageBeforeV16();
};

// Новая ячейка: вместо одинокого '+' менеджер видит понятный статус.
managerCell=function managerCellV16(day,emp,category,view){
  const off=isDayOff(day,emp.id);
  const item=assignmentForManagerCell(day,emp.id,category,view.shift);
  if(off) return `<td class="managerCell offCell" onclick="openEmployeeDrawer('${day}','${emp.id}','','')"><span>Выходной</span></td>`;
  if(!item){
    return `<td class="managerCell unassignedCellV16" onclick="openSkillPicker('${day}','${category}','${view.shift}')"><span>Не назначен</span><small>+ Назначить</small></td>`;
  }
  const changed=item.status==="changed"||item.bySkill||item.category!==emp.position||item.shift!==emp.defaultShift;
  return `<td class="managerCell ${changed?'changedCell':''}" onclick="openEmployeeDrawer('${day}','${emp.id}','${item.category}','${item.shift}')"><b>${shortName(emp.name)}</b><small>${item.shift}</small></td>`;
};

function employeeWeekSummaryV16(empId){
  const days=getWeekDays(state.currentWeek);
  return `<div class="employeeWeekSummaryV16">${days.map((d,i)=>{
    const day=dateKey(d);
    const items=dayItems(day).filter(x=>x.employeeId===empId);
    const off=items.some(x=>x.status==="off");
    const work=items.filter(isWorkItem);
    const hours=work.reduce((s,x)=>s+itemHours(x),0);
    let body="Не назначен";
    if(off) body="🏖 Выходной";
    else if(work.length) body=work.map(x=>`${x.category} · ${x.shift}${x.replacement?' · замена':''}`).join("<br>");
    const tag=off?`<span class="offTagV16">Выходной</span>`:(hours>=FULL_DAY_MIN_HOURS?`<span class="fullTagV16">FULL · ${hours}ч</span>`:"");
    return `<div class="employeeDayLineV16"><b>${daysShort[i]}<br><small>${formatDate(d)}</small></b><div>${body}</div>${tag}</div>`;
  }).join("")}</div>`;
}

// Карточка сотрудника показывает ВСЮ неделю и все зоны/смены, а не только текущую панель.
openManagerBulkActions=function openManagerBulkActionsV16(empId){
  const emp=employee(empId); if(!emp) return;
  const days=getWeekDays(state.currentWeek);
  const first=dateKey(days[0]), last=dateKey(days[6]);
  const drawer=document.getElementById("drawer");
  drawer.innerHTML=`<div class="drawerHead"><div><h2>${shortName(emp.name)}</h2><div class="muted">${emp.position} · основная ${emp.defaultShift}</div></div><button class="drawerClose" onclick="closeDrawer()">✕</button></div>
  <div class="infoBox"><b>Где сотрудник стоит на этой неделе</b><div style="margin-top:10px">${employeeWeekSummaryV16(empId)}</div></div>
  <div class="infoBox"><b>Выберите отдельные дни</b><div class="bulkDays">${days.map((d,i)=>`<label><input type="checkbox" class="managerBulkDay" value="${dateKey(d)}"> ${daysShort[i]} ${formatDate(d)}</label>`).join("")}</div></div>
  <div class="infoBox"><b>Или выберите период</b><div class="periodRowV111"><label>С<input id="bulkPeriodFromV111" type="date" min="${first}" max="${last}" value="${first}"></label><label>По<input id="bulkPeriodToV111" type="date" min="${first}" max="${last}" value="${last}"></label><button class="secondary" onclick="selectBulkPeriodV111()">Выбрать период</button></div></div>
  <div class="drawerActions"><button onclick="managerSetDaysOff('${empId}')">Поставить выходные</button><button class="danger" onclick="managerUnassignDaysV11('${empId}')">Не назначать на выбранные дни</button><button class="secondary" onclick="managerSelectWholeWeek()">Выбрать всю неделю</button><button class="secondary" onclick="managerRestoreDays('${empId}')">Вернуть в график</button></div>`;
  drawer.classList.add("open");
};

function globalEmployeeSearchHtmlV16(){
  const q=String(managerSearch||"").trim().toLowerCase();
  if(!q) return "";
  const matches=state.employees.filter(e=>e.active!==false && e.name.toLowerCase().includes(q)).slice(0,12);
  return `<div class="card globalEmployeeSearchV16"><h3>Поиск по всем сменам и зонам</h3><p class="muted">Сотрудник найдётся, даже если другой менеджер уже поставил его в другую смену или зону.</p>${matches.map(e=>{
    const all=getWeekDays(state.currentWeek).map(dateKey).flatMap(day=>dayItems(day).filter(i=>i.employeeId===e.id&&isWorkItem(i)).map(i=>({day,...i})));
    const info=all.length?all.slice(0,4).map(i=>`${i.day.slice(5)} · ${i.category} · ${i.shift}`).join("<br>"):"На этой неделе пока не назначен";
    return `<div class="searchResultV16"><div><b>${shortName(e.name)}</b><div class="muted">${e.position} · ${e.defaultShift}</div><small>${info}</small></div><button class="secondary" onclick="openManagerBulkActions('${e.id}')">Посмотреть неделю</button></div>`;
  }).join("")||'<div class="empty">Сотрудник не найден</div>'}</div>`;
}

function openManagerAddEmployeeV16(){
  if(state.role!=="manager") return toast("Эта кнопка предназначена для менеджера");
  const drawer=document.getElementById("drawer");
  drawer.innerHTML=`<div class="drawerHead"><div><h2>Добавить сотрудника</h2><div class="muted">Сотрудник добавится в общий список, но смены автоматически не назначаются.</div></div><button class="drawerClose" onclick="closeDrawer()">✕</button></div>
  <div class="managerEmployeeFormV16">
    <label><b>ФИО</b><input id="managerNewNameV16" placeholder="Фамилия Имя Отчество"></label>
    <label><b>Основная должность</b><select id="managerNewPosV16">${editableCategories.filter(c=>c!=="Стажёр"||true).map(c=>`<option>${c}</option>`).join("")}</select></label>
    <label><b>Основная смена</b><select id="managerNewShiftV16">${Object.keys(shiftTemplates).map(s=>`<option>${s}</option>`).join("")}</select></label>
    <div><b>Дополнительные навыки / зоны</b><div class="skillsV16">${editableCategories.map(c=>`<label><input type="checkbox" class="managerNewSkillV16" value="${c}"> ${c}</label>`).join("")}</div></div>
    <button class="managerAddBtnV16" onclick="saveManagerEmployeeV16()">Добавить сотрудника</button>
  </div>`;
  drawer.classList.add("open");
}

function saveManagerEmployeeV16(){
  if(state.role!=="manager") return toast("Нет доступа");
  const name=document.getElementById("managerNewNameV16")?.value.trim();
  const position=document.getElementById("managerNewPosV16")?.value;
  const shift=document.getElementById("managerNewShiftV16")?.value;
  if(!name) return toast("Введите ФИО");
  if(state.employees.some(e=>e.name.trim().toLowerCase()===name.toLowerCase())) return toast("Такой сотрудник уже есть");
  const extra=[...document.querySelectorAll(".managerNewSkillV16:checked")].map(x=>x.value);
  const manager=employee(state.currentManagerId);
  const emp={
    id:`e${Date.now()}`,
    name, position, defaultShift:shift,
    skills:Array.from(new Set([position,...extra])),
    universal:false, active:true,
    serviceScore:serviceRatedCategories.includes(position)?50:null,
    comments:[], fullLimitBlocked:false,
    addedByManagerId:manager?.id||null,
    addedBy:manager?shortName(manager.name):"Менеджер",
  ...
  if(week){
    week.history=week.history||[];
    week.history.push(`${currentActorName()}: добавлен новый сотрудник ${shortName(name)} · ${position} · ${shift}. Смены пока не назначены.`);
  }
  closeDrawer();
  render();
  toast("Сотрудник добавлен. Теперь назначьте нужные дни.");
}

// Дополняем менеджерский экран кнопкой добавления и глобальным поиском.
const renderManagerBoardBeforeV16=renderManagerBoard;
renderManagerBoard=function renderManagerBoardHumanSafeV16(){
  renderManagerBoardBeforeV16();
  const el=document.getElementById("managerBoard"); if(!el) return;
  if(state.role==="manager"){
    const hero=el.querySelector(".managerActions");
    if(hero) hero.insertAdjacentHTML("afterbegin",`<button class="managerAddBtnV16" onclick="openManagerAddEmployeeV16()">+ Добавить сотрудника</button>`);
    const schedule=el.querySelector(".managerSchedule");
    if(schedule && managerSearch) schedule.insertAdjacentHTML("beforebegin",globalEmployeeSearchHtmlV16());
  }
};

// Финальный декор и права доступа.
const renderBeforeFinalV16=render;
render=function renderFinalV16(options={}){
  if(accessModeV16) state.role=accessModeV16;
  renderBeforeFinalV16(options);
  if(accessModeV16) state.role=accessModeV16;
  document.querySelectorAll(".admin-only").forEach(el=>{ el.style.display=state.role==="admin"?"":"none"; });
  const switchBtn=document.getElementById("switchAccessV16");
  if(switchBtn) switchBtn.textContent=state.role==="admin"?"⇄ Выйти из админ-панели":"⇄ Сменить менеджера / вход";

  const top=document.querySelector(".topActions");
  if(top){
    const attendanceManager=currentPage==="attendance"&&state.role==="manager";
    ["prevWeekBtn","weekSelect","nextWeekBtn","excelBtn"].forEach(id=>{
      const node=document.getElementById(id);
      if(node) node.style.display=attendanceManager?"none":"";
    });
  }
};

// Переопределяем навигацию после последнего render.
document.querySelectorAll(".nav").forEach(nav=>{ nav.onclick=()=>navigateV16(nav.dataset.page); });

// На каждом новом открытии сайта стартовый экран остаётся обязательным.
document.getElementById("entryGateV16")?.classList.remove("hidden");

/* ===== Safia Schedule v16.1 Attendance Restore ===== */
function attendancePercentV161(records){
  const total=records.length;
  if(!total) return 0;
  const present=records.filter(x=>x.record?.status==="Вышел"||x.record?.status==="Ушёл раньше").length;
  return Math.round((present/total)*100);
}
function weeklyAttendancePercentV161(){
  ensureAttendance();
  const manager=state.role==="manager"?employee(state.currentManagerId):null;
  let planned=0,present=0,absent=0,early=0,unmarked=0;
  getWeekDays(state.currentWeek).map(dateKey).forEach(day=>{
    let items=dayItems(day).filter(isWorkItem).filter(i=>employee(i.employeeId));
    if(state.role==="manager") items=manager?items.filter(i=>itemMatchesManagerV11(i,manager)):[];
    items.forEach(i=>{
      planned++;
      const rec=state.weeks[state.currentWeek].attendance?.[attendanceKey(day,i.employeeId,i.shift,i.category)];
      if(!rec){unmarked++;return;}
      if(rec.status==="Вышел") present++;
      else if(rec.status==="Ушёл раньше"){present++;early++;}
      else if(rec.status==="Не вышел") absent++;
    });
  });
  return {planned,present,absent,early,unmarked,percent:planned?Math.round((present/planned)*100):0};
}

const openReplacementPickerCandidatesV161=openReplacementPicker;
function markAbsentWithoutReplacementV161(day,absentId,shift,category){
  const comment=prompt("Комментарий к невыходу (необязательно):","")||"";
  setAttendanceWithDetails(day,absentId,shift,category,"Не вышел",{comment,replacementId:null});
  closeDrawer();
}
openReplacementPicker=function openReplacementChoiceV161(day,absentId,shift,category){
  const drawer=document.getElementById("drawer");
  const emp=employee(absentId);
  if(!drawer) return;
  drawer.innerHTML=`<div class="drawerHead"><div><h2>Сотрудник не вышел</h2><div class="muted">${shortName(emp?.name||"")} · ${day} · ${category} · ${shift}</div></div><button class="drawerClose" onclick="closeDrawer()">✕</button></div>
  <div class="absenceChoiceV161">
    <button class="danger absenceChoiceBtnV161" onclick="openReplacementPickerCandidatesV161('${day}','${absentId}','${shift}','${category}')"><b>🔄 Не вышел — выбрать замену</b><span>Подобрать подходящего сотрудника из доступных смен.</span></button>
    <button class="secondary absenceChoiceBtnV161" onclick="markAbsentWithoutReplacementV161('${day}','${absentId}','${shift}','${category}')"><b>❌ Не вышел — без замены</b><span>Зафиксировать невыход. Позиция останется незакрытой.</span></button>
  </div>`;
  drawer.classList.add("open");
};

const renderAttendanceBeforeV161=renderAttendance;
renderAttendance=function renderAttendanceV161(){
  renderAttendanceBeforeV161();
  const target=document.getElementById("attendance");
  if(!target) return;
  ensureAttendance();

  const manager=state.role==="manager"?employee(state.currentManagerId):null;
  let items=dayItems(attendanceSelectedDay).filter(isWorkItem).filter(i=>employee(i.employeeId));
  if(state.role==="manager") items=manager?items.filter(i=>itemMatchesManagerV11(i,manager)):[];
  const records=items.map(item=>({item,record:state.weeks[state.currentWeek].attendance?.[attendanceKey(attendanceSelectedDay,item.employeeId,item.shift,item.category)]}));
  const dayPercent=attendancePercentV161(records);
  const weekly=weeklyAttendancePercentV161();

  const stats=target.querySelector(".attendanceStatsV113");
  if(stats){
    const old=stats.querySelector(".attendancePercentV161"); if(old) old.remove();
    stats.insertAdjacentHTML("afterbegin",`<span class="attendancePercentV161"><small>Посещаемость сегодня</small><b>${dayPercent}%</b></span>`);
  }
  const main=target.querySelector(".attendanceMainCardV113");
  if(main && !target.querySelector(".weeklyAttendanceV161")){
    main.insertAdjacentHTML("afterend",`<div class="card weeklyAttendanceV161"><div><span>Посещаемость за неделю</span><b>${weekly.percent}%</b></div><small>По графику: ${weekly.planned} · вышли: ${weekly.present} · не вышли: ${weekly.absent} · ушли раньше: ${weekly.early} · не отмечены: ${weekly.unmarked}</small></div>`);
  }
};

// Обновляем страницу после загрузки v16.1 без записи в Firebase.
if(typeof render==="function") setTimeout(()=>{ try{ render({save:false}); }catch(e){ console.error("v16.1 render",e); } },0);

/* ===== Safia Schedule v16.3 Access Switcher ===== */
let adminSessionV163 = false;

function setGateVisibleV163(visible){
  const gate=document.getElementById('entryGateV16');
  document.body.classList.toggle('accessLockedV163', !!visible);
  if(gate) gate.classList.toggle('hidden', !visible);
}

// Стартовый экран всегда отдельный: рабочая система под ним не показывается.
setGateVisibleV163(true);

// Обычный вход менеджера — без прав администратора.
enterManagerV16=function enterManagerV163(){
  adminSessionV163=false;
  hideAdminLoginV16();
  accessModeV16='manager';
  state.role='manager';
  state.currentManagerId=null;
  currentPage='managerBoard';
  saveUiSettingsV13();
  setGateVisibleV163(false);
  render({save:false});
  setTimeout(()=>document.getElementById('managerIdentitySelect')?.focus(),80);
  toast('Выберите себя в панели менеджера');
};

// Вход администратора. Пароль проверяется как и раньше, но теперь создаётся отдельная admin-сессия.
enterAdminV16=async function enterAdminV163(){
  const input=document.getElementById('adminPasswordV16');
  const error=document.getElementById('adminLoginErrorV16');
  const hash=await sha256V16(input?.value || '');
  if(hash!==ADMIN_PASSWORD_HASH_V16){
...
function managerOptionsV163(){
  return state.employees
    .filter(e=>e.active!==false&&(e.position==='Менеджер'||e.position==='Менеджер официантской зоны'))
    .sort((a,b)=>a.name.localeCompare(b.name,'ru'));
}

function enterManagerProfileV163(managerId){
  const emp=employee(managerId);
  if(!emp) return toast('Менеджер не найден');
  accessModeV16='manager';
  state.role='manager';
  ...
  closeDrawer();
  render({save:false});
  toast(adminSessionV163?`Просмотр панели: ${shortName(emp.name)}`:`Менеджер: ${shortName(emp.name)}`);
}

function returnToAdminV163(){
  if(!adminSessionV163) return logoutAccessV16();
  accessModeV16='admin';
  state.role='admin';
  state.currentManagerId=null;
  currentPage='managerBoard';
  ...
  closeDrawer();
  render({save:false});
  toast('Возврат в админ-панель');
}

function logoutAccessV163(){
  adminSessionV163=false;
  accessModeV16=null;
  state.currentManagerId=null;
  closeDrawer();
...
logoutAccessV16=logoutAccessV163;

function openAccessSwitcherV163(){
  const drawer=document.getElementById('drawer');
  if(!drawer) return;
  const managers=managerOptionsV163();
  const current=employee(state.currentManagerId);
  if(adminSessionV163){
    drawer.innerHTML=`<div class="drawerHead"><div><h2>Сменить режим</h2><div class="muted">Администратор может открыть любую менеджерскую панель и вернуться обратно без повторного пароля.</div></div><button class="drawerClose" onclick="closeDrawer()">✕</button></div>
      <div class="accessSwitcherV163">
        <button class="accessModeCardV163 adminModeV163" onclick="returnToAdminV163()"><b>🔐 Админ-панель</b><span>Полный доступ ко всем разделам</span></button>
        <div class="infoBox"><b>👤 Панель менеджера</b><p class="muted">Выберите менеджера. Вы увидите сайт так же, как видит он.</p>
          <div class="managerPickListV163">${managers.map(m=>`<button class="managerPickV163 ${current?.id===m.id&&state.role==='manager'?'active':''}" onclick="enterManagerProfileV163('${m.id}')"><b>${shortName(m.name)}</b><small>${typeof managerPanelNameV112 === "function"?managerPanelNameV112(m):((managerPanelKeyV11(m)&&managerViews[managerPanelKeyV11(m)])?managerViews[managerPanelKeyV11(m)].title:m.position)}</small></button>`).join('')}</div>
        </div>
        <button class="danger" onclick="logoutAccessV163()">🚪 Выйти к выбору роли</button>
      </div>`;
  }else{
    drawer.innerHTML=`<div class="drawerHead"><div><h2>Менеджер</h2><div class="muted">Можно выбрать другого менеджера или выйти на стартовый экран.</div></div><button class="drawerClose" onclick="closeDrawer()">✕</button></div>
      <div class="accessSwitcherV163">
        <div class="managerPickListV163">${managers.map(m=>`<button class="managerPickV163 ${current?.id===m.id?'active':''}" onclick="enterManagerProfileV163('${m.id}')"><b>${shortName(m.name)}</b><small>${typeof managerPanelNameV112 === "function"?managerPanelNameV112(m):m.position}</small></button>`).join('')}</div>
        <button class="danger" onclick="logoutAccessV163()">🚪 Выйти к выбору роли</button>
      </div>`;
  }
  drawer.classList.add('open');
...
// Кнопка слева теперь открывает выбор режима, а не выбрасывает администратора из системы.
const switchAccessButtonV163=document.getElementById('switchAccessV16');
if(switchAccessButtonV163) switchAccessButtonV163.onclick=openAccessSwitcherV163;

// Усиливаем финальный render: показываем кнопку возврата, когда админ смотрит глазами менеджера.
const renderBeforeV163=render;
render=function renderV163(options={}){
  renderBeforeV163(options);
  const switchBtn=document.getElementById('switchAccessV16');
  if(switchBtn){
    switchBtn.onclick=openAccessSwitcherV163;
    switchBtn.textContent=adminSessionV163
      ? (state.role==='manager'?'← Вернуться / сменить режим':'⇄ Сменить режим')
      : '⇄ Сменить менеджера / выйти';
  }
  document.querySelectorAll('.adminReturnBarV163').forEach(x=>x.remove());
  if(adminSessionV163 && state.role==='manager'){
    const main=document.querySelector('.main');
    const who=employee(state.currentManagerId);
    if(main){
      main.insertAdjacentHTML('afterbegin',`<div class="adminReturnBarV163"><div><b>🔐 Администратор смотрит панель менеджера</b><span>${who?shortName(who.name):'Менеджер не выбран'}</span></div><button onclick="returnToAdminV163()">← Вернуться в админ-панель</button><button class="secondary" onclick="openAccessSwitcherV163()">Сменить менеджера</button></div>`);
    }
  }
};

// При загрузке стартовый экран остаётся обязательным и полностью закрывает приложение.
setTimeout(()=>setGateVisibleV163(true),0);


/* ===== Safia Schedule v16.4: Менеджер / Управляющий =====
   Внутреннее значение role="admin" сохранено ради совместимости старых модулей.
   В интерфейсе этот режим называется только «Управляющий».
*/
function managerOptionsV164(){
  return state.employees
    .filter(e=>e.active!==false && (e.position==='Менеджер'||e.position==='Менеджер официантской зоны'))
    .sort((a,b)=>a.name.localeCompare(b.name,'ru'));
}

function openManagerEntryPickerV164(){
  const drawer=document.getElementById('drawer');
  if(!drawer) return;
  const managers=managerOptionsV164();
  drawer.innerHTML=`<div class="drawerHead"><div><h2>Выберите себя</h2><div class="muted">Менеджер входит без пароля и работает только в своей панели.</div></div><button class="drawerClose" onclick="closeDrawer();logoutAccessV163()">✕</button></div>
    <div class="accessSwitcherV163"><div class="managerPickListV163">${managers.map(m=>`<button class="managerPickV163" onclick="enterManagerProfileV163('${m.id}')"><b>${shortName(m.name)}</b><small>${typeof managerPanelNameV112 === "function"?managerPanelNameV112(m):m.position}</small></button>`).join('')||'<div class="empty">Менеджеры не найдены</div>'}</div>
    <button class="secondary" onclick="closeDrawer();logoutAccessV163()">← Назад</button></div>`;
  drawer.classList.add('open');
}

// На старте «Менеджер» сразу ведёт к выбору конкретного менеджера.
enterManagerV16=function enterManagerV164(){
  adminSessionV163=false;
  accessModeV16='manager';
  state.role='manager';
  state.currentManagerId=null;
  ...
  setGateVisibleV163(false);
  render({save:false});
  openManagerEntryPickerV164();
};

// После правильного пароля — сразу полная Управляющая панель. Никакого выбора роли.
enterAdminV16=async function enterManagerControllerV164(){
  const input=document.getElementById('adminPasswordV16');
  const error=document.getElementById('adminLoginErrorV16');
  const hash=await sha256V16(input?.value || '');
  if(hash!==ADMIN_PASSWORD_HASH_V16){
  ...
  setGateVisibleV163(false);
  render({save:false});
  toast('Управляющая панель: полный доступ');
};

function returnToAdminV163(){
  if(!adminSessionV163) return logoutAccessV163();
  accessModeV16='admin';
  state.role='admin';
  state.currentManagerId=null;
  currentPage='managerBoard';
  ...
  closeDrawer();
  render({save:false});
  toast('Возврат в управляющую панель');
}

// Управляющий может открыть менеджерскую панель, но это не меняет его авторизацию.
openAccessSwitcherV163=function openControllerSwitcherV164(){
  const drawer=document.getElementById('drawer');
  if(!drawer) return;
  const managers=managerOptionsV164();
  const current=employee(state.currentManagerId);
  if(adminSessionV163){
    drawer.innerHTML=`<div class="drawerHead"><div><h2>Управляющая панель</h2><div class="muted">Можно открыть любую менеджерскую панель для проверки и вернуться без повторного пароля.</div></div><button class="drawerClose" onclick="closeDrawer()">✕</button></div>
      <div class="accessSwitcherV163">
        <button class="accessModeCardV163 adminModeV163" onclick="returnToAdminV163()"><b>🔐 Управляющая панель</b><span>Полный доступ ко всем разделам</span></button>
        <div class="infoBox"><b>👤 Открыть как менеджер</b><p class="muted">Выберите менеджера.</p><div class="managerPickListV163">${managers.map(m=>`<button class="managerPickV163 ${current?.id===m.id&&state.role==='manager'?'active':''}" onclick="enterManagerProfileV163('${m.id}')"><b>${shortName(m.name)}</b><small>${typeof managerPanelNameV112 === "function"?managerPanelNameV112(m):m.position}</small></button>`).join('')}</div></div>
        <button class="danger" onclick="logoutAccessV163()">🚪 Выйти</button>
      </div>`;
  }else{
    drawer.innerHTML=`<div class="drawerHead"><div><h2>Менеджер</h2><div class="muted">Выберите свою панель или выйдите.</div></div><button class="drawerClose" onclick="closeDrawer()">✕</button></div>
      <div class="accessSwitcherV163"><div class="managerPickListV163">${managers.map(m=>`<button class="managerPickV163 ${current?.id===m.id?'active':''}" onclick="enterManagerProfileV163('${m.id}')"><b>${shortName(m.name)}</b><small>${typeof managerPanelNameV112 === "function"?managerPanelNameV112(m):m.position}</small></button>`).join('')}</div><button class="danger" onclick="logoutAccessV163()">🚪 Выйти</button></div>`;
  }
  drawer.classList.add('open');
};

// Последний UI-слой: термин «Администратор» не показываем пользователю.
const renderBeforeV164=render;
render=function renderV164(options={}){
  renderBeforeV164(options);
  const switchBtn=document.getElementById('switchAccessV16');
  if(switchBtn){
    switchBtn.onclick=openAccessSwitcherV163;
    switchBtn.textContent=adminSessionV163 ? (state.role==='manager'?'← Управляющая панель / сменить':'⇄ Открыть как менеджер') : '⇄ Сменить менеджера / выйти';
  }
  document.querySelectorAll('.adminReturnBarV163').forEach(x=>x.remove());
  if(adminSessionV163 && state.role==='manager'){
    const main=document.querySelector('.main');
    const who=employee(state.currentManagerId);
    if(main) main.insertAdjacentHTML('afterbegin',`<div class="adminReturnBarV163"><div><b>🔐 Управляющий смотрит панель менеджера</b><span>${who?shortName(who.name):'Менеджер не выбран'}</span></div><button onclick="returnToAdminV163()">← Вернуться в управляющую панель</button><button class="secondary" onclick="openAccessSwitcherV163()">Сменить менеджера</button></div>`);
  }
  // Скрытый select нужен старому коду, но пользователь его не видит.
  const roleBox=document.querySelector('.roleBox');
  const roleLabel=roleBox?.querySelector('label');
  if(roleLabel) roleLabel.textContent=state.role==='admin'?'Управляющий':'Менеджер';
};

setTimeout(()=>setGateVisibleV163(true),0);

/* ===== Safia Schedule v17 · Employee Self-Service ===== */
let employeeSelfIdV17 = null;
let employeeRequestSearchV17 = "";
let employeeRequestStatusV17 = "Все";

function ensureWeekForV17(weekKey){
  const previous=addDays(weekKey,-7);
  if(state.weeks?.[weekKey]){
    const actualWeek=dateKey(weekStart(new Date()));
    if(weekKey>actualWeek && state.weeks?.[previous]) repairInheritedDayOffGapsV181(weekKey,previous);
    return state.weeks[weekKey];
  }
  createWeek(weekKey,state.weeks?.[previous]?previous:null);
  return state.weeks[weekKey];
}
function prefStoreV17(weekKey){
  const week=ensureWeekForV17(weekKey);
  week.employeePreferences=week.employeePreferences||{};
  return week.employeePreferences;
}
function prefRecordV17(weekKey,empId){
  const store=prefStoreV17(weekKey);
  store[empId]=store[empId]||{offDays:[],availableDays:[],fullDays:[],submittedAt:null,updatedAt:null};
  store[empId].offDays=Array.isArray(store[empId].offDays)?store[empId].offDays:[];
  store[empId].availableDays=Array.isArray(store[empId].availableDays)?store[empId].availableDays:[];
  store[empId].fullDays=Array.isArray(store[empId].fullDays)?store[empId].fullDays:[];
  return store[empId];
}
function employeeTargetWeekV17(now=new Date()){
  const current=weekStart(now);
  let target=new Date(current); target.setDate(target.getDate()+7);
  let targetKey=dateKey(target);
  let deadline=new Date(target); deadline.setDate(deadline.getDate()-4); deadline.setHours(23,59,59,999);
  if(now>deadline){
    target.setDate(target.getDate()+7); targetKey=dateKey(target);
    deadline=new Date(target); deadline.setDate(deadline.getDate()-4); deadline.setHours(23,59,59,999);
  }
  return {weekKey:targetKey,deadline};
}
function deadlineForWeekV17(weekKey){
  const d=new Date(weekKey); d.setDate(d.getDate()-4); d.setHours(23,59,59,999); return d;
}
function isPreferenceOpenV17(weekKey){ return new Date()<=deadlineForWeekV17(weekKey); }
function humanDateTimeV17(d){ return d.toLocaleString('ru-RU',{day:'2-digit',month:'2-digit',hour:'2-digit',minute:'2-digit'}); }
function weekDayLabelV17(day,index){ return `${daysShort[index]} · ${formatDate(new Date(day))}`; }
function itemsForWeekDayV17(weekKey,day){ return state.weeks?.[weekKey]?.schedule?.[day]||[]; }
function offRequestersV17(weekKey,day,ignoreEmpId=null){
  const prefs=state.weeks?.[weekKey]?.employeePreferences||{};
  return new Set(Object.entries(prefs).filter(([id,p])=>id!==ignoreEmpId&&(p?.offDays||[]).includes(day)).map(([id])=>id));
}
function projectedCoverageV17(weekKey,day,category,block,ignoreEmpId=null,removeCandidateId=null){
  const offIds=offRequestersV17(weekKey,day,ignoreEmpId);
  if(removeCandidateId) offIds.add(removeCandidateId);
  return itemsForWeekDayV17(weekKey,day).filter(i=>isWorkItem(i)&&i.category===category&&coversShift(i,block)>0&&!offIds.has(i.employeeId)).length;
}
function dayOffEligibilityV17(weekKey,empId,day){
  const emp=employee(empId); if(!emp) return {ok:false,reason:'Сотрудник не найден'};
  const work=itemsForWeekDayV17(weekKey,day).filter(i=>i.employeeId===empId&&isWorkItem(i));
  if(!work.length) return {ok:true,reason:'В этот день вы не стоите в графике'};
  for(const item of work){
    for(const block of Object.keys(staffingBlocks)){
      if(coversShift(item,block)<=0) continue;
      const needed=state.weeks?.[weekKey]?.staffing?.[day]?.[item.category]?.[block]??0;
      const after=projectedCoverageV17(weekKey,day,item.category,block,empId,empId);
      if(after<needed) return {ok:false,reason:`Нельзя выбрать: ${item.category}, ${block} — после выходного будет ${after}/${needed}`};
    }
  }
  return {ok:true,reason:'Штатка позволяет выбрать выходной'};
}
function enterEmployeeV17(){
  adminSessionV163=false;
  accessModeV16=null;
  employeeSelfIdV17=null;
  hideAdminLoginV16(); setGateVisibleV163(false);
  document.body.classList.add('employeeModeV17');
  document.getElementById('employeePortalV17')?.classList.add('active');
  renderEmployeePickerV17();
}
function exitEmployeeV17(){
  employeeSelfIdV17=null;
  document.body.classList.remove('employeeModeV17');
  document.getElementById('employeePortalV17')?.classList.remove('active');
  setGateVisibleV163(true);
}
function employeeSearchTextV172(value){
  const map={
  ...
  return String(value||'').trim().toLowerCase().split('').map(ch=>map[ch]??ch).join('').replace(/[^a-z0-9]+/g,' ');
}
function renderEmployeePickerV17(filter=''){
  const root=document.getElementById('employeePortalContentV17'); if(!root) return;
  const q=employeeSearchTextV172(filter);
  const employees=(state.employees||[])
    .filter(e=>e&&e.active!==false&&e.position!=='Менеджер'&&e.position!=='Менеджер официантской зоны')
    .filter(e=>!q||employeeSearchTextV172(e.name).includes(q)||employeeSearchTextV172(e.phone).includes(q)||employeeSearchTextV172(e.position).includes(q))
    .sort((a,b)=>String(a.name||'').localeCompare(String(b.name||''),'ru'));
  root.innerHTML=`<div class="card"><h2>Выберите себя</h2><p class="muted">Найдите себя по ФИО. После входа вы увидите свой график: в какие дни выходите, в какой зоне и в какую смену.</p><input style="width:100%" placeholder="Поиск по ФИО" oninput="renderEmployeePickerV17(this.value)" value="${String(filter).replace(/"/g,'&quot;')}"><div class="employeePickerGridV17">${employees.map(e=>`<button class="employeePickerV17" onclick="openEmployeeSelfV17('${e.id}')"><b>${String(e.name||'Без имени')}</b><small>${e.position||'Без должности'} · ${e.defaultShift||'смена не указана'}</small></button>`).join('')||'<div class="empty">Ничего не найдено. Попробуйте ввести фамилию латиницей или кириллицей.</div>'}</div></div>`;
}
function openEmployeeSelfV17(empId){ employeeSelfIdV17=empId; renderEmployeeSelfV17(); }
function setEmployeePhoneV17(empId,value){
  const emp=employee(empId); if(!emp)return;
  emp.phone=String(value||'').trim(); saveState({immediate:true}); toast('Номер сохранён'); renderEmployeeSelfV17();
}
function toggleEmployeePreferenceV17(empId,day,type,checked){
  const {weekKey}=employeeTargetWeekV17();
  if(!isPreferenceOpenV17(weekKey)) return toast('Выбор на эту неделю уже закрыт');
  const pref=prefRecordV17(weekKey,empId);
  if(type==='off'&&checked){
    const eligibility=dayOffEligibilityV17(weekKey,empId,day); if(!eligibility.ok){ toast(eligibility.reason); return renderEmployeeSelfV17(); }
    pref.availableDays=pref.availableDays.filter(d=>d!==day); pref.fullDays=pref.fullDays.filter(d=>d!==day);
  }
  if((type==='available'||type==='full')&&checked) pref.offDays=pref.offDays.filter(d=>d!==day);
  const key=type==='off'?'offDays':type==='available'?'availableDays':'fullDays';
  if(checked&&!pref[key].includes(day)) pref[key].push(day);
  if(!checked) pref[key]=pref[key].filter(d=>d!==day);
  pref.updatedAt=new Date().toISOString();
  saveState({immediate:true}); renderEmployeeSelfV17();
}
function submitEmployeePreferencesV17(empId){
  const {weekKey}=employeeTargetWeekV17(); if(!isPreferenceOpenV17(weekKey))return toast('Срок выбора уже закончился');
  const pref=prefRecordV17(weekKey,empId); pref.submittedAt=new Date().toISOString(); pref.updatedAt=pref.submittedAt;
  state.weeks[weekKey].history=state.weeks[weekKey].history||[];
  state.weeks[weekKey].history.push(`${shortName(employee(empId)?.name)} отправил выбор: выходные ${pref.offDays.join(', ')||'—'}; может выйти ${pref.availableDays.join(', ')||'—'}; FULL ${pref.fullDays.join(', ')||'—'}`);
  saveState({immediate:true}); renderEmployeeSelfV17(); toast('Выбор отправлен менеджеру');
}
function employeeScheduleForWeekV172(empId,weekKey){
  const days=getWeekDays(weekKey).map(dateKey);
  return `<div class="employeeScheduleWeekV172"><div class="employeeScheduleWeekHeadV172"><b>Неделя ${weekLabel(weekKey)}</b></div>${days.map((day,i)=>{
    const items=itemsForWeekDayV17(weekKey,day).filter(x=>x.employeeId===empId&&isWorkItem(x));
    const off=isDayOff(day,empId);
    let body='';
    if(off){
      body=`<span class="employeeScheduleOffV172">🏖 Выходной</span>`;
    }else if(!items.length){
      body=`<span class="muted">⏳ Не назначен — график ещё формируется</span>`;
    }else{
      body=items.map(item=>`<div class="employeeScheduleAssignmentV172"><b>${item.category||'Зона не указана'}</b><span>🕒 ${item.shift||'—'}</span>${item.replacement?'<small>🔄 Замена</small>':''}${item.additionalShift?'<small>⚡ Дополнительная смена</small>':''}</div>`).join('');
    }
    return `<div class="employeeScheduleDayV172"><div class="employeeScheduleDateV172"><b>${daysShort[i]}</b><small>${formatDate(new Date(day))}</small></div><div>${body}</div></div>`;
  }).join('')}</div>`;
}
function employeeOwnScheduleBlockV172(empId,targetWeekKey){
  const currentWeekKey=dateKey(weekStart(new Date()));
  const keys=[currentWeekKey];
  if(targetWeekKey!==currentWeekKey) keys.push(targetWeekKey);
  keys.forEach(ensureWeekForV17);
  return `<div class="card employeeOwnScheduleV172"><div class="employeeOwnScheduleTitleV172"><div><h3>🗓 Мой график</h3><p class="muted">Здесь видно, когда вы выходите, в какую смену и в какой зоне. Изменения менеджера будут обновляться автоматически.</p></div></div>${keys.map(k=>employeeScheduleForWeekV172(empId,k)).join('')}</div>`;
}
function renderEmployeeSelfV17(){
  const root=document.getElementById('employeePortalContentV17'), emp=employee(employeeSelfIdV17); if(!root||!emp)return renderEmployeePickerV17();
  const target=employeeTargetWeekV17(), weekKey=target.weekKey; ensureWeekForV17(weekKey);
  const pref=prefRecordV17(weekKey,emp.id), open=isPreferenceOpenV17(weekKey), days=getWeekDays(weekKey).map(dateKey);
  root.innerHTML=`<button class="secondary" onclick="employeeSelfIdV17=null;renderEmployeePickerV17()">← Выбрать другого сотрудника</button>
  <div class="employeeSelfHeroV17" style="margin-top:12px"><div class="employeeSelfInfoV17"><div class="eyebrow">МОЙ ПРОФИЛЬ</div><h2>${shortName(emp.name)}</h2><p>${emp.position} · постоянная смена ${emp.defaultShift}</p><div class="employeePhoneBoxV17"><input id="employeePhoneV17" type="tel" placeholder="Номер телефона" value="${String(emp.phone||'').replace(/"/g,'&quot;')}"><button onclick="setEmployeePhoneV17('${emp.id}',document.getElementById('employeePhoneV17').value)">Сохранить номер</button></div></div>
  <div class="employeeDeadlineV17 ${open?'':'locked'}"><b>${open?'✅ Выбор открыт':'🔒 Выбор закрыт'}</b><p>Неделя ${weekLabel(weekKey)}</p><small>До четверга · ${humanDateTimeV17(target.deadline)}</small>${pref.submittedAt?`<p><b>Отправлено:</b> ${humanDateTimeV17(new Date(pref.submittedAt))}</p>`:'<p class="muted">Вы ещё не нажали «Отправить менеджеру».</p>'}</div></div>
  ${employeeOwnScheduleBlockV172(emp.id,weekKey)}
  <div class="prefLegendV17"><span>🏖 Выходной</span><span>✅ Могу выйти</span><span>⚡ Могу FULL</span></div>
  <div class="employeeDaysGridV17">${days.map((day,i)=>{const elig=dayOffEligibilityV17(weekKey,emp.id,day),off=pref.offDays.includes(day),avail=pref.availableDays.includes(day),full=pref.fullDays.includes(day);return `<div class="employeeDayCardV17"><h4>${weekDayLabelV17(day,i)}</h4><small>${emp.defaultShift}</small><label class="prefCheckV17 ${!elig.ok&&!off?'disabled':''}"><input type="checkbox" ${off?'checked':''} ${(!open||(!elig.ok&&!off))?'disabled':''} onchange="toggleEmployeePreferenceV17('${emp.id}','${day}','off',this.checked)"> 🏖 Хочу выходной</label>${!elig.ok&&!off?`<div class="staffWarningV17">${elig.reason}</div>`:''}<label class="prefCheckV17"><input type="checkbox" ${avail?'checked':''} ${open?'':'disabled'} onchange="toggleEmployeePreferenceV17('${emp.id}','${day}','available',this.checked)"> ✅ Могу выйти</label><label class="prefCheckV17"><input type="checkbox" ${full?'checked':''} ${open?'':'disabled'} onchange="toggleEmployeePreferenceV17('${emp.id}','${day}','full',this.checked)"> ⚡ Могу FULL</label></div>`}).join('')}</div>
  <div class="card" style="margin-top:14px"><b>Важно</b><p class="muted">Вы выбираете только выходной, доступность и FULL. Должность и постоянную смену меняет только менеджер/управляющий. Выходной недоступен, если после него штатка станет ниже нормы.</p>${open?`<button class="darkBtn" onclick="submitEmployeePreferencesV17('${emp.id}')">Отправить менеджеру</button>`:''}</div>`;
}
function prefDaysTextV17(days,weekKey,cls){
  const weekDays=getWeekDays(weekKey).map(dateKey); return (days||[]).sort().map(d=>`<span class="prefBadgeV17 ${cls}">${daysShort[weekDays.indexOf(d)]||d}</span>`).join('')||'<span class="muted">—</span>';
}
function activeEmployeesForRequestsV17(){ return state.employees.filter(e=>e.active&&e.position!=='Менеджер'&&e.position!=='Менеджер официантской зоны'); }
function renderEmployeeRequestsV17(){
  const el=document.getElementById('employeeRequests'); if(!el)return;
  const weekKey=state.currentWeek, week=ensureWeekForV17(weekKey), prefs=week.employeePreferences||{}, employees=activeEmployeesForRequestsV17();
  const submitted=employees.filter(e=>prefs[e.id]?.submittedAt).length, missing=employees.length-submitted;
  const canWork=employees.filter(e=>(prefs[e.id]?.availableDays||[]).length).length, canFull=employees.filter(e=>(prefs[e.id]?.fullDays||[]).length).length;
  let rows=employees.filter(e=>!employeeRequestSearchV17||e.name.toLowerCase().includes(employeeRequestSearchV17.toLowerCase())||String(e.phone||'').includes(employeeRequestSearchV17));
  if(employeeRequestStatusV17==='Не выбрали') rows=rows.filter(e=>!prefs[e.id]?.submittedAt);
  if(employeeRequestStatusV17==='Выбрали') rows=rows.filter(e=>prefs[e.id]?.submittedAt);
  if(employeeRequestStatusV17==='Могут выйти') rows=rows.filter(e=>(prefs[e.id]?.availableDays||[]).length);
  if(employeeRequestStatusV17==='Могут FULL') rows=rows.filter(e=>(prefs[e.id]?.fullDays||[]).length);
  rows.sort((a,b)=>Number(!!prefs[a.id]?.submittedAt)-Number(!!prefs[b.id]?.submittedAt)||a.name.localeCompare(b.name,'ru'));
  el.innerHTML=`<div class="managerHero"><div><div class="eyebrow">САМОСТОЯТЕЛЬНЫЙ ВЫБОР</div><h2>Выбор сотрудников</h2><p>Неделя ${weekLabel(weekKey)} · срок до ${humanDateTimeV17(deadlineForWeekV17(weekKey))}</p></div></div>
  <div class="preferenceSummaryV17"><div class="statCard"><span>Выбрали</span><b>${submitted}</b></div><div class="statCard lowStat"><span>Не выбрали</span><b>${missing}</b></div><div class="statCard"><span>Могут выйти</span><b>${canWork}</b></div><div class="statCard highStat"><span>Могут FULL</span><b>${canFull}</b></div></div>
  <div class="card"><div class="preferenceFiltersV17"><input placeholder="Поиск ФИО / телефон" value="${employeeRequestSearchV17.replace(/"/g,'&quot;')}" oninput="employeeRequestSearchV17=this.value;renderEmployeeRequestsV17()"><select onchange="employeeRequestStatusV17=this.value;renderEmployeeRequestsV17()">${['Все','Не выбрали','Выбрали','Могут выйти','Могут FULL'].map(x=>`<option ${employeeRequestStatusV17===x?'selected':''}>${x}</option>`).join('')}</select></div><div style="overflow:auto"><table class="prefTableV17"><thead><tr><th>Сотрудник</th><th>Телефон</th><th>Выходные</th><th>Могу выйти</th><th>FULL</th><th>Статус</th></tr></thead><tbody>${rows.map(e=>{const p=prefs[e.id]||{};return `<tr><td><b>${shortName(e.name)}</b><br><small>${e.position} · ${e.defaultShift}</small></td><td>${e.phone?`<b>${e.phone}</b>`:'<span class="muted">не указан</span>'}</td><td>${prefDaysTextV17(p.offDays,weekKey,'off')}</td><td>${prefDaysTextV17(p.availableDays,weekKey,'available')}</td><td>${prefDaysTextV17(p.fullDays,weekKey,'full')}</td><td>${p.submittedAt?`<span class="prefBadgeV17 available">Выбрал</span><br><small>${humanDateTimeV17(new Date(p.submittedAt))}</small>`:'<span class="prefBadgeV17 missing">Не выбрал</span>'}</td></tr>`}).join('')||'<tr><td colspan="6" class="muted">Нет сотрудников по фильтру</td></tr>'}</tbody></table></div></div>`;
}
function preferencePriorityV17(emp,day){
  const p=state.weeks?.[state.currentWeek]?.employeePreferences?.[emp.id]||{};
  if((p.offDays||[]).includes(day)) return 99;
  if((p.availableDays||[]).includes(day)) return 0;
  if((p.fullDays||[]).includes(day)) return 1;
  return 2;
}
function helperBadgesV17(emp,day){
  const p=state.weeks?.[state.currentWeek]?.employeePreferences?.[emp.id]||{}, out=[];
  if((p.availableDays||[]).includes(day)) out.push('<span class="prefBadgeV17 available">✅ Может выйти</span>');
  if((p.fullDays||[]).includes(day)) out.push('<span class="prefBadgeV17 full">⚡ Может FULL</span>');
  return out.join('');
}
function addFromPreferenceHelperV17(day,category,shiftName,empId){
  const e=employee(empId); if(!e)return;
  const defaultShift=shiftName==='1 смена'?'08:00-16:00':shiftName==='2 смена'?'16:00-23:00':'23:00-08:00';
  const proposed=(state.weeks?.[state.currentWeek]?.employeePreferences?.[empId]?.fullDays||[]).includes(day)
    ? (shiftName==='1 смена'?'08:00-20:00':shiftName==='2 смена'?'11:00-23:00':e.defaultShift)
    : defaultShift;
  const shift=prompt('Время работы:',proposed); if(!shift||!shiftInfo(shift))return toast('Неверный формат времени');
  const check=validateReplacementCandidate(e,day,shift,category); if(!check.ok)return toast(check.reason);
  removeOffForEmployee(day,empId);
  state.weeks[state.currentWeek].schedule[day].push({employeeId:empId,category,shift,status:'changed',bySkill:category!==e.position,additionalShift:hoursForEmployeeOnDayAnyWeek(empId,day)>0,comment:'Добавлен по шпаргалке доступности'});
  state.weeks[state.currentWeek].history.push(`${currentActorName()}: ${shortName(e.name)} добавлен по шпаргалке доступности — ${category}, ${shift}, ${day}`);
  closeDrawer(); render(); toast('Сотрудник добавлен');
}
openSkillPicker=function openSkillPickerV17(day,category,shiftName){
  const candidates=state.employees.filter(e=>e.active&&canWorkCategory(e,category)&&!isDayOff(day,e.id)).map(e=>({emp:e,priority:preferencePriorityV17(e,day)})).filter(x=>x.priority<99).sort((a,b)=>a.priority-b.priority||shortName(a.emp.name).localeCompare(shortName(b.emp.name),'ru'));
  const drawer=document.getElementById('drawer'); if(!drawer)return;
  drawer.innerHTML=`<div class="drawerHead"><div><h2>Кого поставить?</h2><div class="muted">${day} · ${category} · ${shiftName}</div></div><button class="drawerClose" onclick="closeDrawer()">✕</button></div><div class="infoBox"><b>💡 Шпаргалка менеджера</b><p class="muted">Сначала сотрудники, которые сами отметили «могу выйти», затем «могу FULL». Сотрудники, запросившие выходной, исключены.</p></div><div>${candidates.map(({emp,priority})=>`<div class="helperCandidateV17 ${priority===0?'priorityAvailable':priority===1?'priorityFull':''}"><div><b>${shortName(emp.name)}</b><div class="muted">${emp.position} · ${emp.defaultShift}</div><div class="meta">${helperBadgesV17(emp,day)}${priority===2?'<span class="prefBadgeV17">Нет отметки</span>':''}</div></div><div><div class="phone">${emp.phone||'📞 нет номера'}</div><button style="margin-top:7px" onclick="addFromPreferenceHelperV17('${day}','${category}','${shiftName}','${emp.id}')">Поставить</button></div></div>`).join('')||'<div class="infoBox">Нет доступных сотрудников.</div>'}</div>`;
  drawer.classList.add('open');
}

const renderBeforeV17=render;
render=function renderV17(options={}){
  renderBeforeV17(options);
  renderEmployeeRequestsV17();
  const title=document.getElementById('pageTitle');
  if(title&&currentPage==='employeeRequests') title.textContent='Выбор сотрудников';
};

// При обычном выходе всегда закрываем и сотруднический режим.
const logoutAccessBeforeV17=logoutAccessV163;
logoutAccessV163=function logoutAccessV17(){
  document.body.classList.remove('employeeModeV17');
  document.getElementById('employeePortalV17')?.classList.remove('active');
  employeeSelfIdV17=null;
  return logoutAccessBeforeV17();
};
logoutAccessV16=logoutAccessV163;

/* ===== Safia Schedule v18 · Availability + protected staffing ===== */
function ensureV18Settings(){
  state.staffingToleranceV18=state.staffingToleranceV18||{};
  editableCategories.forEach(cat=>{
    state.staffingToleranceV18[cat]=state.staffingToleranceV18[cat]||{};
    Object.keys(staffingBlocks).forEach(block=>{
      if(!Number.isFinite(Number(state.staffingToleranceV18[cat][block]))) state.staffingToleranceV18[cat][block]=1;
    });
  });
}
function staffingToleranceV18(category,block){
  ensureV18Settings();
  return Math.max(0,Number(state.staffingToleranceV18?.[category]?.[block]??1)||0);
}
function setStaffingToleranceV18(category,block,value){
  ensureV18Settings();
  state.staffingToleranceV18[category][block]=Math.max(0,Math.min(99,Number(value)||0));
  saveState({immediate:true}); renderEmployeeRequestsV17(); toast('Допуск сохранён');
}
function minimumStaffingV18(weekKey,day,category,block){
  const needCount=Number(state.weeks?.[weekKey]?.staffing?.[day]?.[category]?.[block]??0)||0;
  const tolerance=staffingToleranceV18(category,block);
  return {needed:needCount,tolerance,minimum:Math.max(0,needCount-tolerance)};
}
function uniqueCoverageV18(weekKey,day,category,block,excludeEmpId=null){
  const ids=new Set();
  (state.weeks?.[weekKey]?.schedule?.[day]||[]).forEach(item=>{
    if(!isWorkItem(item)||item.category!==category||item.employeeId===excludeEmpId) return;
    if(coversShift(item,block)>0) ids.add(item.employeeId);
  });
  return ids.size;
}
function dayOffEligibilityV18(weekKey,empId,day){
  const emp=employee(empId); if(!emp) return {ok:false,status:'red',reason:'Сотрудник не найден',lines:[]};
  const work=(state.weeks?.[weekKey]?.schedule?.[day]||[]).filter(i=>i.employeeId===empId&&isWorkItem(i));
  if(!work.length){
    const alreadyOff=(state.weeks?.[weekKey]?.schedule?.[day]||[]).some(i=>i.employeeId===empId&&i.status==='off');
    return {ok:true,status:alreadyOff?'yellow':'green',reason:alreadyOff?'Выходной уже стоит':'В этот день нет рабочего назначения',lines:[]};
  }
  let status='green'; const lines=[];
  for(const item of work){
    for(const block of Object.keys(staffingBlocks)){
      if(coversShift(item,block)<=0) continue;
      const cfg=minimumStaffingV18(weekKey,day,item.category,block);
      const after=uniqueCoverageV18(weekKey,day,item.category,block,empId);
      if(after<cfg.minimum){
        lines.push(`${item.category}, ${block}: после выходного ${after}, минимум ${cfg.minimum} (штатка ${cfg.needed})`);
        return {ok:false,status:'red',reason:`Нельзя выбрать выходной — ${item.category}, ${block}: останется ${after}, минимально допустимо ${cfg.minimum}`,lines};
      }
      if(after<cfg.needed){
        status='yellow';
        lines.push(`${item.category}, ${block}: останется ${after}/${cfg.needed}, это допустимый минимум`);
      }
    }
  }
  return {ok:true,status,reason:status==='yellow'?'Можно выбрать, но смена останется на допустимом минимуме':'Штатка позволяет выбрать выходной',lines};
}
dayOffEligibilityV17=dayOffEligibilityV18;

function prefRecordV18(weekKey,empId){
  const pref=prefRecordV17(weekKey,empId);
  pref.timeAvailability=(pref.timeAvailability&&typeof pref.timeAvailability==='object')?pref.timeAvailability:{};
  pref.backupAssignments=(pref.backupAssignments&&typeof pref.backupAssignments==='object')?pref.backupAssignments:{};
  pref.managerAssignedOffDays=Array.isArray(pref.managerAssignedOffDays)?pref.managerAssignedOffDays:[];
  return pref;
}
function syncEmployeeOffToScheduleV18(weekKey,empId,day,makeOff){
  const week=ensureWeekForV17(weekKey), pref=prefRecordV18(weekKey,empId);
  week.schedule[day]=Array.isArray(week.schedule[day])?week.schedule[day]:[];
  if(makeOff){
    const work=week.schedule[day].filter(i=>i.employeeId===empId&&isWorkItem(i));
    if(work.length&&!pref.backupAssignments[day]) pref.backupAssignments[day]=JSON.parse(JSON.stringify(work));
    week.schedule[day]=week.schedule[day].filter(i=>i.employeeId!==empId);
    week.schedule[day].push({employeeId:empId,status:'off',source:'employeePreferenceV18'});
  }else{
    week.schedule[day]=week.schedule[day].filter(i=>!(i.employeeId===empId&&i.status==='off'));
    const already=week.schedule[day].some(i=>i.employeeId===empId&&isWorkItem(i));
    if(!already){
      const backup=Array.isArray(pref.backupAssignments[day])?pref.backupAssignments[day]:[];
      if(backup.length) week.schedule[day].push(...JSON.parse(JSON.stringify(backup)));
      else{
        const emp=employee(empId);
        if(emp) week.schedule[day].push({employeeId:empId,category:emp.position,shift:emp.defaultShift,status:'work'});
      }
    }
  }
}

toggleEmployeePreferenceV17=function toggleEmployeePreferenceV18(empId,day,type,checked){
  const {weekKey}=employeeTargetWeekV17();
  if(!isPreferenceOpenV17(weekKey)) return toast('Выбор на эту неделю уже закрыт');
  const pref=prefRecordV18(weekKey,empId);
  if(type==='off'){
    if(checked){
      const eligibility=dayOffEligibilityV18(weekKey,empId,day);
      if(!eligibility.ok){ toast(eligibility.reason); return renderEmployeeSelfV17(); }
      if(!pref.offDays.includes(day)) pref.offDays.push(day);
      syncEmployeeOffToScheduleV18(weekKey,empId,day,true);
    }else{
      pref.offDays=pref.offDays.filter(d=>d!==day);
      pref.managerAssignedOffDays=pref.managerAssignedOffDays.filter(d=>d!==day);
      syncEmployeeOffToScheduleV18(weekKey,empId,day,false);
    }
  }else{
    const key=type==='available'?'availableDays':'fullDays';
    if(checked&&!pref[key].includes(day)) pref[key].push(day);
    if(!checked) pref[key]=pref[key].filter(d=>d!==day);
  }
  // Изменение после отправки снова требует подтверждения сотрудником.
  pref.submittedAt=null; pref.submittedBy=null; pref.updatedAt=new Date().toISOString();
  saveState({immediate:true}); renderEmployeeSelfV17();
};

function saveEmployeeTimeAvailabilityV18(empId,day,index){
  const {weekKey}=employeeTargetWeekV17();
  if(!isPreferenceOpenV17(weekKey)) return toast('Выбор на эту неделю уже закрыт');
  const pref=prefRecordV18(weekKey,empId);
  const enabled=document.getElementById(`timeAvailEnabledV18_${index}`)?.checked;
  if(!enabled){ delete pref.timeAvailability[day]; pref.submittedAt=null; saveState({immediate:true}); renderEmployeeSelfV17(); return toast('Время удалено'); }
  const start=document.getElementById(`timeAvailStartV18_${index}`)?.value||'';
  const end=document.getElementById(`timeAvailEndV18_${index}`)?.value||'';
  const range=`${start}-${end}`, info=shiftInfo(range);
  if(!start||!end||!info) return toast('Укажите корректное время');
  if(info.hours>17) return toast('Больше 17 часов выбрать нельзя');
  pref.timeAvailability[day]={start,end,shift:range,hours:info.hours};
  pref.submittedAt=null; pref.submittedBy=null; pref.updatedAt=new Date().toISOString();
  saveState({immediate:true}); renderEmployeeSelfV17(); toast(`Доступность сохранена: ${range}`);
}
function toggleTimeAvailInputsV18(index,checked){
  ['Start','End'].forEach(k=>{const el=document.getElementById(`timeAvail${k}V18_${index}`); if(el) el.disabled=!checked;});
}
function availabilityTextV18(pref,day){
  const t=pref?.timeAvailability?.[day];
  return t?.shift||'';
}
function primaryStaffingBlockV18(emp){
  let best='1 смена',score=-1;
  Object.keys(staffingBlocks).forEach(block=>{const x=coversShift({shift:emp.defaultShift},block); if(x>score){score=x;best=block;}});
  return best;
}

employeeScheduleForWeekV172=function employeeScheduleForWeekV18(empId,weekKey){
  const days=getWeekDays(weekKey).map(dateKey), pref=state.weeks?.[weekKey]?.employeePreferences?.[empId]||{};
  return `<div class="employeeScheduleWeekV172"><div class="employeeScheduleWeekHeadV172"><b>Неделя ${weekLabel(weekKey)}</b></div>${days.map((day,i)=>{
    const all=state.weeks?.[weekKey]?.schedule?.[day]||[];
    const items=all.filter(x=>x.employeeId===empId&&isWorkItem(x));
    const off=all.some(x=>x.employeeId===empId&&x.status==='off');
    let body='';
    if(off||!items.length){
      body=`<span class="employeeScheduleOffV172">🏖 Выходной</span>${(pref.availableDays||[]).includes(day)?'<span class="prefBadgeV17 available">📞 Могу выйти при необходимости</span>':''}${(pref.fullDays||[]).includes(day)?'<span class="prefBadgeV17 full">⚡ Могу FULL</span>':''}${availabilityTextV18(pref,day)?`<span class="prefBadgeV18 time">🕒 ${availabilityTextV18(pref,day)}</span>`:''}`;
    }else{
      body=items.map(item=>`<div class="employeeScheduleAssignmentV172"><b>${item.category||'Зона не указана'}</b><span>🕒 ${item.shift||'—'}</span>${item.replacement?'<small>🔄 Замена</small>':''}${item.additionalShift?'<small>⚡ Дополнительная смена</small>':''}</div>`).join('');
    }
    return `<div class="employeeScheduleDayV172"><div class="employeeScheduleDateV172"><b>${daysShort[i]}</b><small>${formatDate(new Date(day))}</small></div><div>${body}</div></div>`;
...
renderEmployeeSelfV17=function renderEmployeeSelfV18(){
  const root=document.getElementById('employeePortalContentV17'), emp=employee(employeeSelfIdV17); if(!root||!emp)return renderEmployeePickerV17();
  const target=employeeTargetWeekV17(), weekKey=target.weekKey; ensureWeekForV17(weekKey);
  const pref=prefRecordV18(weekKey,emp.id), open=isPreferenceOpenV17(weekKey), days=getWeekDays(weekKey).map(dateKey);
  root.innerHTML=`<button class="secondary" onclick="employeeSelfIdV17=null;renderEmployeePickerV17()">← Выбрать другого сотрудника</button>
  <div class="employeeSelfHeroV17" style="margin-top:12px"><div class="employeeSelfInfoV17"><div class="eyebrow">МОЙ ПРОФИЛЬ</div><h2>${shortName(emp.name)}</h2><p>${emp.position} · постоянная смена ${emp.defaultShift}</p><div class="employeePhoneBoxV17"><input id="employeePhoneV17" type="tel" placeholder="Номер телефона" value="${String(emp.phone||'').replace(/"/g,'&quot;')}"><button onclick="setEmployeePhoneV17('${emp.id}',document.getElementById('employeePhoneV17').value)">Сохранить номер</button></div></div>
  <div class="employeeDeadlineV17 ${open?'':'locked'}"><b>${open?'✅ Выбор открыт':'🔒 Выбор закрыт'}</b><p>Неделя ${weekLabel(weekKey)}</p><small>До четверга · ${humanDateTimeV17(target.deadline)}</small>${pref.submittedAt?`<p><b>Отправлено:</b> ${humanDateTimeV17(new Date(pref.submittedAt))}${pref.submittedBy==='manager'?' · менеджером':''}</p>`:'<p class="muted">После выбора нажмите «Отправить менеджеру».</p>'}</div></div>
  ${employeeOwnScheduleBlockV172(emp.id,weekKey)}
  <div class="prefLegendV17"><span>🏖 Нужен выходной</span><span>📞 Могу выйти даже в выходной</span><span>⚡ Могу FULL</span><span>🕒 Могу в другое время</span></div>
  <div class="employeeDaysGridV17">${days.map((day,i)=>{const elig=dayOffEligibilityV18(weekKey,emp.id,day),off=pref.offDays.includes(day),avail=pref.availableDays.includes(day),full=pref.fullDays.includes(day),t=pref.timeAvailability?.[day];return `<div class="employeeDayCardV17 staffing-${off?'selected':elig.status}"><h4>${weekDayLabelV17(day,i)}</h4><small>${emp.defaultShift}</small><div class="staffingLightV18 ${off?'selected':elig.status}">${off?'🏖 Выходной выбран':elig.status==='green'?'🟢 Выходной доступен':elig.status==='yellow'?'🟡 Можно, останется минимум':'🔴 Выходной недоступен'}</div><label class="prefCheckV17 ${!elig.ok&&!off?'disabled':''}"><input type="checkbox" ${off?'checked':''} ${(!open||(!elig.ok&&!off))?'disabled':''} onchange="toggleEmployeePreferenceV17('${emp.id}','${day}','off',this.checked)"> 🏖 Нужен выходной</label>${!elig.ok&&!off?`<div class="staffWarningV17">${elig.reason}</div>`:elig.status==='yellow'&&!off?`<div class="staffWarningV18">${elig.reason}</div>`:''}<label class="prefCheckV17"><input type="checkbox" ${avail?'checked':''} ${open?'':'disabled'} onchange="toggleEmployeePreferenceV17('${emp.id}','${day}','available',this.checked)"> 📞 Могу выйти при необходимости</label><label class="prefCheckV17"><input type="checkbox" ${full?'checked':''} ${open?'':'disabled'} onchange="toggleEmployeePreferenceV17('${emp.id}','${day}','full',this.checked)"> ⚡ Могу FULL</label><div class="timeAvailBoxV18"><label><input id="timeAvailEnabledV18_${i}" type="checkbox" ${t?'checked':''} ${open?'':'disabled'} onchange="toggleTimeAvailInputsV18(${i},this.checked)"> 🕒 Другое время</label><div class="timeAvailInputsV18"><input id="timeAvailStartV18_${i}" type="time" value="${t?.start||'18:00'}" ${t&&open?'':'disabled'}><span>—</span><input id="timeAvailEndV18_${i}" type="time" value="${t?.end||'02:00'}" ${t&&open?'':'disabled'}></div>${open?`<button class="tinyBtnV18" onclick="saveEmployeeTimeAvailabilityV18('${emp.id}','${day}',${i})">Сохранить время</button>`:''}</div></div>`}).join('')}</div>
  <div class="card" style="margin-top:14px"><b>Как это работает</b><p class="muted">Выходной можно выбрать только пока смена не опускается ниже минимальной штатки. «Могу выйти» можно отметить вместе с выходным — тогда менеджер увидит, что вам можно позвонить при необходимости. FULL и другое время — подсказка менеджеру, а не автоматическое назначение.</p>${open?`<button class="darkBtn" onclick="submitEmployeePreferencesV17('${emp.id}')">Отправить менеджеру</button>`:''}</div>`;
};

submitEmployeePreferencesV17=function submitEmployeePreferencesV18(empId){
  const {weekKey}=employeeTargetWeekV17(); if(!isPreferenceOpenV17(weekKey))return toast('Срок выбора уже закончился');
  const pref=prefRecordV18(weekKey,empId); pref.submittedAt=new Date().toISOString(); pref.submittedBy='employee'; pref.updatedAt=pref.submittedAt;
  state.weeks[weekKey].history=state.weeks[weekKey].history||[];
  const timeText=Object.entries(pref.timeAvailability||{}).map(([d,t])=>`${d} ${t.shift}`).join(', ')||'—';
  state.weeks[weekKey].history.push(`${shortName(employee(empId)?.name)} отправил выбор: выходные ${pref.offDays.join(', ')||'—'}; можно вызвать ${pref.availableDays.join(', ')||'—'}; FULL ${pref.fullDays.join(', ')||'—'}; другое время ${timeText}`);
  saveState({immediate:true}); renderEmployeeSelfV17(); toast('Выбор отправлен менеджеру');
};

function managerMissingGroupsV18(employees,prefs){
  const groups={'1 смена':[],'2 смена':[],'3 смена':[]};
  employees.filter(e=>!prefs[e.id]?.submittedAt).forEach(e=>groups[primaryStaffingBlockV18(e)].push(e));
  Object.values(groups).forEach(list=>list.sort((a,b)=>a.name.localeCompare(b.name,'ru')));
  return groups;
}
function renderMissingGroupV18(block,list){
  return `<div class="missingShiftV18"><div class="missingShiftHeadV18"><div><b>${block}</b><span>${list.length} не выбрали</span></div></div>${list.length?list.map(e=>`<div class="missingEmployeeV18"><div><b>${shortName(e.name)}</b><small>${e.position} · ${e.defaultShift}</small></div><button onclick="openManagerAssignOffV18('${e.id}')">Назначить выходной</button></div>`).join(''):'<div class="allDoneV18">✅ Все выбрали</div>'}</div>`;
}
function staffingTolerancePanelV18(){
  ensureV18Settings();
  const editable=state.role==='admin';
  return `<details class="card tolerancePanelV18"><summary><b>🛡 Минимальная штатка</b> · по умолчанию разрешён минус 1</summary><p class="muted">Например: штатка 12 и допуск 1 → меньше 11 человек выходной выбрать нельзя.${editable?' Управляющий может изменить допуск для каждой позиции и смены.':''}</p><div class="toleranceGridV18">${editableCategories.map(cat=>`<div class="toleranceRowV18"><b>${cat}</b>${Object.keys(staffingBlocks).map(block=>`<label>${block}<input type="number" min="0" max="9" value="${staffingToleranceV18(cat,block)}" ${editable?'':'disabled'} onchange="setStaffingToleranceV18('${cat}','${block}',this.value)"></label>`).join('')}</div>`).join('')}</div></details>`;
}
function prefTimeTextV18(pref,weekKey){
  const weekDays=getWeekDays(weekKey).map(dateKey), entries=Object.entries(pref?.timeAvailability||{});
  return entries.map(([d,t])=>`<span class="prefBadgeV18 time">${daysShort[weekDays.indexOf(d)]||d} ${t.shift}</span>`).join('')||'<span class="muted">—</span>';
}
renderEmployeeRequestsV17=function renderEmployeeRequestsV18(){
  const el=document.getElementById('employeeRequests'); if(!el)return;
  const weekKey=state.currentWeek, week=ensureWeekForV17(weekKey), prefs=week.employeePreferences||{}, employees=activeEmployeesForRequestsV17();
  const submitted=employees.filter(e=>prefs[e.id]?.submittedAt).length, missing=employees.length-submitted;
  const canWork=employees.filter(e=>(prefs[e.id]?.availableDays||[]).length).length, canFull=employees.filter(e=>(prefs[e.id]?.fullDays||[]).length).length;
  const groups=managerMissingGroupsV18(employees,prefs);
  let rows=employees.filter(e=>!employeeRequestSearchV17||e.name.toLowerCase().includes(employeeRequestSearchV17.toLowerCase())||String(e.phone||'').includes(employeeRequestSearchV17));
  if(employeeRequestStatusV17==='Не выбрали') rows=rows.filter(e=>!prefs[e.id]?.submittedAt);
  if(employeeRequestStatusV17==='Выбрали') rows=rows.filter(e=>prefs[e.id]?.submittedAt);
  if(employeeRequestStatusV17==='Могут выйти') rows=rows.filter(e=>(prefs[e.id]?.availableDays||[]).length);
  if(employeeRequestStatusV17==='Могут FULL') rows=rows.filter(e=>(prefs[e.id]?.fullDays||[]).length);
  rows.sort((a,b)=>Number(!!prefs[a.id]?.submittedAt)-Number(!!prefs[b.id]?.submittedAt)||a.name.localeCompare(b.name,'ru'));
  el.innerHTML=`<div class="managerHero"><div><div class="eyebrow">ПОДГОТОВКА ГРАФИКА</div><h2>Выбор сотрудников</h2><p>Неделя ${weekLabel(weekKey)} · сотрудникам доступно до ${humanDateTimeV17(deadlineForWeekV17(weekKey))}</p></div></div>
  <div class="preferenceSummaryV17"><div class="statCard"><span>Выбрали</span><b>${submitted}</b></div><div class="statCard lowStat"><span>Не выбрали</span><b>${missing}</b></div><div class="statCard"><span>Могут выйти</span><b>${canWork}</b></div><div class="statCard highStat"><span>Могут FULL</span><b>${canFull}</b></div></div>
  <div class="card"><h3>Кто ещё не выбрал выходной</h3><p class="muted">Список разделён по основной смене. Менеджер может назначить выходной сам — защита минимальной штатки действует и здесь.</p><div class="missingGroupsV18">${Object.keys(groups).map(block=>renderMissingGroupV18(block,groups[block])).join('')}</div></div>
  ${staffingTolerancePanelV18()}
  <div class="card"><div class="preferenceFiltersV17"><input placeholder="Поиск ФИО / телефон" value="${employeeRequestSearchV17.replace(/"/g,'&quot;')}" oninput="employeeRequestSearchV17=this.value;renderEmployeeRequestsV17()"><select onchange="employeeRequestStatusV17=this.value;renderEmployeeRequestsV17()">${['Все','Не выбрали','Выбрали','Могут выйти','Могут FULL'].map(x=>`<option ${employeeRequestStatusV17===x?'selected':''}>${x}</option>`).join('')}</select></div><div style="overflow:auto"><table class="prefTableV17"><thead><tr><th>Сотрудник</th><th>Телефон</th><th>Выходные</th><th>Можно вызвать</th><th>FULL</th><th>Другое время</th><th>Статус</th></tr></thead><tbody>${rows.map(e=>{const p=prefs[e.id]||{};return `<tr><td><b>${shortName(e.name)}</b><br><small>${e.position} · ${e.defaultShift}</small></td><td>${e.phone?`<b>${e.phone}</b>`:'<span class="muted">не указан</span>'}</td><td>${prefDaysTextV17(p.offDays,weekKey,'off')}</td><td>${prefDaysTextV17(p.availableDays,weekKey,'available')}</td><td>${prefDaysTextV17(p.fullDays,weekKey,'full')}</td><td>${prefTimeTextV18(p,weekKey)}</td><td>${p.submittedAt?`<span class="prefBadgeV17 available">${p.submittedBy==='manager'?'Назначил менеджер':'Выбрал'}</span><br><small>${humanDateTimeV17(new Date(p.submittedAt))}</small>`:'<span class="prefBadgeV17 missing">Не выбрал</span>'}</td></tr>`}).join('')||'<tr><td colspan="7" class="muted">Нет сотрудников по фильтру</td></tr>'}</tbody></table></div></div>`;
};

function openManagerAssignOffV18(empId){
  const emp=employee(empId), weekKey=state.currentWeek; if(!emp)return;
  const pref=prefRecordV18(weekKey,empId), days=getWeekDays(weekKey).map(dateKey), drawer=document.getElementById('drawer'); if(!drawer)return;
  drawer.innerHTML=`<div class="drawerHead"><div><h2>Назначить выходной</h2><div class="muted">${shortName(emp.name)} · ${emp.position} · ${emp.defaultShift}</div></div><button class="drawerClose" onclick="closeDrawer()">✕</button></div><div class="infoBox"><b>Защита штатки включена</b><p class="muted">Красный день выбрать нельзя. Жёлтый можно выбрать — после него останется допустимый минимум.</p></div><div class="managerOffDaysV18">${days.map((day,i)=>{const elig=dayOffEligibilityV18(weekKey,empId,day),checked=pref.offDays.includes(day);return `<label class="managerOffDayV18 ${checked?'selected':elig.status}"><input class="managerOffChoiceV18" data-day="${day}" type="checkbox" ${checked?'checked':''} ${(!elig.ok&&!checked)?'disabled':''}><b>${daysShort[i]} · ${formatDate(new Date(day))}</b><span>${checked?'🏖 Уже выходной':elig.status==='green'?'🟢 Можно':elig.status==='yellow'?'🟡 Допустимый минимум':'🔴 Нельзя'}</span><small>${elig.reason}</small></label>`}).join('')}</div><button class="darkBtn" style="margin-top:14px" onclick="saveManagerAssignedOffV18('${empId}')">Сохранить выходные</button>`;
  drawer.classList.add('open');
}
function saveManagerAssignedOffV18(empId){
  const weekKey=state.currentWeek, pref=prefRecordV18(weekKey,empId), selected=[...document.querySelectorAll('.managerOffChoiceV18:checked')].map(x=>x.dataset.day);
  const oldManager=Array.isArray(pref.managerAssignedOffDays)?[...pref.managerAssignedOffDays]:[];
  // Проверяем каждый новый выходной ещё раз перед сохранением.
  for(const day of selected){
    if(pref.offDays.includes(day)) continue;
    const e=dayOffEligibilityV18(weekKey,empId,day); if(!e.ok) return toast(e.reason);
  }
  oldManager.filter(d=>!selected.includes(d)).forEach(day=>{pref.offDays=pref.offDays.filter(x=>x!==day);syncEmployeeOffToScheduleV18(weekKey,empId,day,false);});
  selected.forEach(day=>{if(!pref.offDays.includes(day)){pref.offDays.push(day);syncEmployeeOffToScheduleV18(weekKey,empId,day,true);}});
  pref.managerAssignedOffDays=selected; pref.submittedAt=new Date().toISOString(); pref.submittedBy='manager'; pref.updatedAt=pref.submittedAt;
  state.weeks[weekKey].history=state.weeks[weekKey].history||[];
  state.weeks[weekKey].history.push(`${currentActorName()}: назначены выходные ${shortName(employee(empId)?.name)} — ${selected.join(', ')||'—'}`);
  saveState({immediate:true}); closeDrawer(); render(); toast('Выходные сотрудника сохранены');
}

function preferencePriorityV18(emp,day){
  const p=state.weeks?.[state.currentWeek]?.employeePreferences?.[emp.id]||{};
  const off=(p.offDays||[]).includes(day), available=(p.availableDays||[]).includes(day), full=(p.fullDays||[]).includes(day), custom=!!p.timeAvailability?.[day];
  if(off&&!available&&!full&&!custom) return 99;
  if(available) return 0;
  if(full) return 1;
  if(custom) return 2;
  return 3;
}
preferencePriorityV17=preferencePriorityV18;
helperBadgesV17=function helperBadgesV18(emp,day){
  const p=state.weeks?.[state.currentWeek]?.employeePreferences?.[emp.id]||{}, out=[];
  if((p.offDays||[]).includes(day)) out.push('<span class="prefBadgeV17 off">🏖 Выходной</span>');
  if((p.availableDays||[]).includes(day)) out.push('<span class="prefBadgeV17 available">📞 Можно вызвать</span>');
  if((p.fullDays||[]).includes(day)) out.push('<span class="prefBadgeV17 full">⚡ Может FULL</span>');
  if(p.timeAvailability?.[day]) out.push(`<span class="prefBadgeV18 time">🕒 ${p.timeAvailability[day].shift}</span>`);
  return out.join('');
};
addFromPreferenceHelperV17=function addFromPreferenceHelperV18(day,category,shiftName,empId){
  const e=employee(empId); if(!e)return;
  const p=state.weeks?.[state.currentWeek]?.employeePreferences?.[empId]||{};
  const defaultShift=shiftName==='1 смена'?'08:00-16:00':shiftName==='2 смена'?'16:00-23:00':'23:00-08:00';
  let proposed=defaultShift;
  if(p.timeAvailability?.[day]?.shift) proposed=p.timeAvailability[day].shift;
  else if((p.fullDays||[]).includes(day)) proposed=shiftName==='1 смена'?'08:00-20:00':shiftName==='2 смена'?'11:00-23:00':e.defaultShift;
  const shift=prompt('Время работы:',proposed); if(!shift||!shiftInfo(shift))return toast('Неверный формат времени');
  const check=validateReplacementCandidate(e,day,shift,category); if(!check.ok)return toast(check.reason);
  removeOffForEmployee(day,empId);
  state.weeks[state.currentWeek].schedule[day].push({employeeId:empId,category,shift,status:'changed',bySkill:category!==e.position,additionalShift:hoursForEmployeeOnDayAnyWeek(empId,day)>0,comment:'Добавлен по шпаргалке v18'});
  state.weeks[state.currentWeek].history.push(`${currentActorName()}: ${shortName(e.name)} добавлен по шпаргалке — ${category}, ${shift}, ${day}`);
  closeDrawer(); render(); toast('Сотрудник добавлен');
};
openSkillPicker=function openSkillPickerV18(day,category,shiftName){
  const candidates=state.employees.filter(e=>e.active&&canWorkCategory(e,category)).map(e=>({emp:e,priority:preferencePriorityV18(e,day)})).filter(x=>x.priority<99).sort((a,b)=>a.priority-b.priority||shortName(a.emp.name).localeCompare(shortName(b.emp.name),'ru'));
  const drawer=document.getElementById('drawer'); if(!drawer)return;
  drawer.innerHTML=`<div class="drawerHead"><div><h2>Кого поставить?</h2><div class="muted">${day} · ${category} · ${shiftName}</div></div><button class="drawerClose" onclick="closeDrawer()">✕</button></div><div class="infoBox"><b>💡 Шпаргалка менеджера</b><p class="muted">Сначала те, кто отметил «могу выйти», затем FULL, затем своё доступное время. Сотрудник с жёстким выходным без готовности выйти не показывается.</p></div><div>${candidates.map(({emp,priority})=>`<div class="helperCandidateV17 ${priority===0?'priorityAvailable':priority===1?'priorityFull':priority===2?'priorityTimeV18':''}"><div><b>${shortName(emp.name)}</b><div class="muted">${emp.position} · ${emp.defaultShift}</div><div class="meta">${helperBadgesV17(emp,day)}${priority===3?'<span class="prefBadgeV17">Нет отметки</span>':''}</div></div><div><div class="phone">${emp.phone||'📞 нет номера'}</div><button style="margin-top:7px" onclick="addFromPreferenceHelperV17('${day}','${category}','${shiftName}','${emp.id}')">Поставить</button></div></div>`).join('')||'<div class="infoBox">Нет доступных сотрудников.</div>'}</div>`;
  drawer.classList.add('open');
};

// Перерисовываем новую страницу после каждого общего render.
// При загрузке v18.1 один раз проверяем открытую неделю и убираем только
// унаследованные пустые дни, если сотрудники ещё не отправляли выбор.
function autoRepairCurrentWeekV181(){
  const key=state?.currentWeek; if(!key||!state.weeks?.[key]) return false;
  const actualWeek=dateKey(weekStart(new Date()));
  if(key<=actualWeek) return false; // прошлые и текущую фактическую неделю не трогаем
  const prev=addDays(key,-7); if(!state.weeks?.[prev]) return false;
  return repairInheritedDayOffGapsV181(key,prev);
}

const renderBeforeV18=render;
render=function renderV18(options={}){
  const repairedV181=autoRepairCurrentWeekV181();
  if(repairedV181) saveState({immediate:true});
  renderBeforeV18(options);
  ensureV18Settings();
  renderEmployeeRequestsV17();
  const title=document.getElementById('pageTitle'); if(title&&currentPage==='employeeRequests') title.textContent='Выбор сотрудников и доступность';
};

/* ===== Safia Schedule v18.2 · Employee day-off sync fix =====
   Fixes a case where employee preference checkboxes were saved, but the
   manager schedule still showed a work assignment / "Не назначен" instead of
   the selected day off. Employee preference remains the source of truth.
*/
function prefRecordV182(weekKey, empId){
  const p = prefRecordV18(weekKey, empId);
  p.scheduleOverrideDays = Array.isArray(p.scheduleOverrideDays) ? p.scheduleOverrideDays : [];
  return p;
}

function reconcileEmployeeOffDaysV182(weekKey, empId, options={}){
  const week = ensureWeekForV17(weekKey);
  const pref = prefRecordV182(weekKey, empId);
  const days = getWeekDays(weekKey).map(dateKey);
  let changed = false;

  days.forEach(day=>{
    week.schedule[day] = Array.isArray(week.schedule?.[day]) ? week.schedule[day] : [];
    const wantsOff = pref.offDays.includes(day);
    const managerOverride = pref.scheduleOverrideDays.includes(day);
    const hasOffMarker = week.schedule[day].some(i=>i.employeeId===empId && i.status==='off');
    const hasWork = week.schedule[day].some(i=>i.employeeId===empId && isWorkItem(i));

    if(wantsOff && !managerOverride){
      if(!hasOffMarker || hasWork){
        syncEmployeeOffToScheduleV18(weekKey, empId, day, true);
        changed = true;
      }
    } else if(!wantsOff && hasOffMarker){
      // Remove only preference-created day off and restore the employee's work.
      syncEmployeeOffToScheduleV18(weekKey, empId, day, false);
      changed = true;
    }
  });
...
// Selecting/unselecting a day off clears a previous manager override for that day.
const toggleEmployeePreferenceBeforeV182 = toggleEmployeePreferenceV17;
toggleEmployeePreferenceV17 = function toggleEmployeePreferenceV182(empId, day, type, checked){
  const {weekKey} = employeeTargetWeekV17();
  const pref = prefRecordV182(weekKey, empId);
  if(type==='off') pref.scheduleOverrideDays = pref.scheduleOverrideDays.filter(d=>d!==day);
  const result = toggleEmployeePreferenceBeforeV182(empId, day, type, checked);
  // Ensure local schedule and preference never drift apart.
  if(type==='off'){
    reconcileEmployeeOffDaysV182(weekKey, empId);
    saveState({immediate:true});
  }
  return result;
};

// On final submit, force a second reconciliation before saving to Firebase.
submitEmployeePreferencesV17 = function submitEmployeePreferencesV182(empId){
  const {weekKey}=employeeTargetWeekV17();
  if(!isPreferenceOpenV17(weekKey)) return toast('Срок выбора уже закончился');
  const pref=prefRecordV182(weekKey,empId);
  reconcileEmployeeOffDaysV182(weekKey, empId);
  pref.submittedAt=new Date().toISOString();
  pref.submittedBy='employee';
  pref.updatedAt=pref.submittedAt;
  state.weeks[weekKey].history=state.weeks[weekKey].history||[];
  const timeText=Object.entries(pref.timeAvailability||{}).map(([d,t])=>`${d} ${t.shift}`).join(', ')||'—';
  state.weeks[weekKey].history.push(`${shortName(employee(empId)?.name)} отправил выбор: выходные ${pref.offDays.join(', ')||'—'}; можно вызвать ${pref.availableDays.join(', ')||'—'}; FULL ${pref.fullDays.join(', ')||'—'}; другое время ${timeText}`);
  saveState({immediate:true});
  renderEmployeeSelfV17();
  toast('Выходные и доступность сохранены в графике');
};

// If a manager deliberately calls an employee on their requested day off,
// keep the preference visible but do not auto-remove the new assignment.
addFromPreferenceHelperV17 = function addFromPreferenceHelperV182(day,category,shiftName,empId){
  const e=employee(empId); if(!e)return;
  const p=prefRecordV182(state.currentWeek,empId);
  const defaultShift=shiftName==='1 смена'?'08:00-16:00':shiftName==='2 смена'?'16:00-23:00':'23:00-08:00';
  let proposed=defaultShift;
  if(p.timeAvailability?.[day]?.shift) proposed=p.timeAvailability[day].shift;
  else if((p.fullDays||[]).includes(day)) proposed=shiftName==='1 смена'?'08:00-20:00':shiftName==='2 смена'?'11:00-23:00':e.defaultShift;
  ...
  const check=validateReplacementCandidate(e,day,shift,category); if(!check.ok)return toast(check.reason);
  removeOffForEmployee(day,empId);
  if(!p.scheduleOverrideDays.includes(day)) p.scheduleOverrideDays.push(day);
  state.weeks[state.currentWeek].schedule[day].push({employeeId:empId,category,shift,status:'changed',bySkill:category!==e.position,additionalShift:hoursForEmployeeOnDayAnyWeek(empId,day)>0,comment:'Добавлен по шпаргалке v18.2'});
  state.weeks[state.currentWeek].history.push(`${currentActorName()}: ${shortName(e.name)} вызван в выбранный выходной/по доступности — ${category}, ${shift}, ${day}`);
  saveState({immediate:true});
  closeDrawer(); render(); toast('Сотрудник добавлен');
};

// Repair already submitted employee choices from v18/v18.1 once the manager
// opens that future week. This does not touch manager override days.
function repairSubmittedPreferencesV182(weekKey){
  const week=state.weeks?.[weekKey]; if(!week) return false;
  let changed=false;
  Object.entries(week.employeePreferences||{}).forEach(([empId,p])=>{
    if(!p?.submittedAt) return;
    if(reconcileEmployeeOffDaysV182(weekKey,empId)) changed=true;
  });
  if(changed){
    week.history=week.history||[];
    week.history.push('v18.2: синхронизированы выбранные сотрудниками выходные с рабочим графиком');
  }
  return changed;
}

const renderBeforeV182 = render;
render = function renderV182(options={}){
  const key=state?.currentWeek;
  if(key && state.weeks?.[key] && repairSubmittedPreferencesV182(key)) saveState({immediate:true});
  renderBeforeV182(options);
};
