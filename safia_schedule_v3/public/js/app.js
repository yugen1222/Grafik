...
function openAccessSwitcherV163(){
  ...
  if(adminSessionV163){
    drawer.innerHTML=`<div class="drawerHead"><div><h2>Сменить режим</h2><div class="muted">Администратор может открыть любую менеджерскую панель и вернуться обратно без повторного пароля.</div></div><button class="drawerClose" onclick="closeDrawer()">✕</button></div>
      <div class="accessSwitcherV163">
        ...
        <div class="infoBox"><b>👤 Панель менеджера</b><p class="muted">Выберите менеджера. Вы увидите сайт так же, как видит он.</p>
          <div class="managerPickListV163">${managers.map(m=>`<button class="managerPickV163 ${current?.id===m.id&&state.role==='manager'?'active':''}" onclick="enterManagerProfileV163('${m.id}')"><b>${shortName(m.name)}</b><small>${managerPanelNameV112?managerPanelNameV112(m):((managerPanelKeyV11(m)&&managerViews[managerPanelKeyV11(m)])?managerViews[managerPanelKeyV11(m)].title:m.position)}</small></button>`).join('')}</div>
        </div>
    ...
    drawer.innerHTML=`<div class="drawerHead"><div><h2>Менеджер</h2><div class="muted">Можно выбрать другого менеджера или выйти на стартовый экран.</div></div><button class="drawerClose" onclick="closeDrawer()">✕</button></div>
      <div class="accessSwitcherV163">
        <div class="managerPickListV163">${managers.map(m=>`<button class="managerPickV163 ${current?.id===m.id?'active':''}" onclick="enterManagerProfileV163('${m.id}')"><b>${shortName(m.name)}</b><small>${managerPanelNameV112?managerPanelNameV112(m):m.position}</small></button>`).join('')}</div>
        <button class="danger" onclick="logoutAccessV163()">🚪 Выйти к выбору роли</button>
      </div>`;
  }
  drawer.classList.add('open');
...
function openManagerEntryPickerV164(){
  ...
  drawer.innerHTML=`<div class="drawerHead"><div><h2>Выберите себя</h2><div class="muted">Менеджер входит без пароля и работает только в своей панели.</div></div><button class="drawerClose" onclick="closeDrawer();logoutAccessV163()">✕</button></div>
    <div class="accessSwitcherV163"><div class="managerPickListV163">${managers.map(m=>`<button class="managerPickV163" onclick="enterManagerProfileV163('${m.id}')"><b>${shortName(m.name)}</b><small>${managerPanelNameV112?managerPanelNameV112(m):m.position}</small></button>`).join('')||'<div class="empty">Менеджеры не найдены</div>'}</div>
  ...
  if(adminSessionV163){
    drawer.innerHTML=`<div class="drawerHead"><div><h2>Управляющая панель</h2><div class="muted">Можно открыть любую менеджерскую панель для проверки и вернуться без повторного пароля.</div></div><button class="drawerClose" onclick="closeDrawer()">✕</button></div>
      <div class="accessSwitcherV163">
        ...
        <div class="infoBox"><b>👤 Открыть как менеджер</b><p class="muted">Выберите менеджера.</p><div class="managerPickListV163">${managers.map(m=>`<button class="managerPickV163 ${current?.id===m.id&&state.role==='manager'?'active':''}" onclick="enterManagerProfileV163('${m.id}')"><b>${shortName(m.name)}</b><small>${managerPanelNameV112?managerPanelNameV112(m):m.position}</small></button>`).join('')}</div></div>
    ...
    drawer.innerHTML=`<div class="drawerHead"><div><h2>Менеджер</h2><div class="muted">Выберите свою панель или выйдите.</div></div><button class="drawerClose" onclick="closeDrawer()">✕</button></div>
      <div class="accessSwitcherV163"><div class="managerPickListV163">${managers.map(m=>`<button class="managerPickV163 ${current?.id===m.id?'active':''}" onclick="enterManagerProfileV163('${m.id}')"><b>${shortName(m.name)}</b><small>${managerPanelNameV112?managerPanelNameV112(m):m.position}</small></button>`).join('')}</div><button class="danger" onclick="logoutAccessV163()">🚪 Выйти</button></div>`;
  }
  drawer.classList.add('open');
};
