...
// ================= SAFIA SCHEDULE V14.2 STABLE FIX: ЗАЩИТА ДАННЫХ И АВТОВОССТАНОВЛЕНИЕ =================
...
function startFirebaseSyncV14(){
  ...
  cloudLoadedV13=false;
  cloudWriteLockedV14=true;
  showBootV13('Загружаем рабочие данные…');
  ...
  // V14.1 читает только рабочий state. Backups больше не скачиваются при каждом открытии.
  stateRef.on('value',async snapshot=>{
    const rawState=snapshot.val();
    const validation=validateCloudStateV14(rawState);

    if(!validation.ok){
      showBootV13(`Рабочие данные не прошли проверку: ${validation.reason}. Ищем резервную копию…`);
      const restored=await restoreLatestBackupV14(databaseRef);
      if(restored){
        setSyncStatusV112('База восстановлена из резервной копии','ok');
        return;
      }
      showFatalCloudV14('Firebase не содержит безопасной рабочей копии. Ничего не записано. Импортируйте сохранённый JSON.');
      firstSnapshot=false;
      return;
    }
    ...
    try{
      ...
      if(!migratedValidation.ok) throw new Error(migratedValidation.reason);
      ...
      remote.currentWeek=(ui.currentWeek && remote.weeks?.[ui.currentWeek])
        ? ui.currentWeek
        : (remote.currentWeek && remote.weeks?.[remote.currentWeek] ? remote.currentWeek : (weekKeys.pop() || dateKey(weekStart(new Date()))));
      delete remote.__cloudPlaceholder;
      ...
      isRemoteUpdate=false;
      console.error('V14.1 render/load error',err);
      showFatalCloudV14(`Не удалось обработать данные: ${err.message||err}`);
    }
  },error=>{
