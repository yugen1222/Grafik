...
function renderEmployeePickerV17(filter=''){
  ...
  const employees=state.employees.filter(e=>e.active&&e.position!=='Менеджер'&&e.position!=='Менеджер официантской зоны'&&(!q||e.name.toLowerCase().includes(q)||String(e.phone||'').includes(q))).sort((a,b)=>a.name.localeCompare(b.name,'ru'));
  root.innerHTML=`<div class="card"><h2>Выберите себя</h2><p class="muted">Здесь можно указать выходные, дни когда вы можете выйти, дни для FULL и свой номер телефона. Должность и смену изменить нельзя.</p><input style="width:100%" placeholder="Поиск по ФИО" oninput="renderEmployeePickerV17(this.value)" value="${filter.replace(/"/g,'&quot;')}"><div class="employeePickerGridV17">${employees.map(e=>`<button class="employeePickerV17" onclick="openEmployeeSelfV17('${e.id}')"><b>${shortName(e.name)}</b><small>${e.position} · ${e.defaultShift}</small></button>`).join('')||'<div class="empty">Ничего не найдено</div>'}</div></div>`;
}
