...
function copiedScheduleForNewWeekV171(startKey, copyFromKey){
  ...
  days.forEach((day, index) => {
    const oldItems = state.weeks?.[copyFromKey]?.schedule?.[oldDays[index]] || [];
    const activeIds = new Set(state.employees.filter(e=>e.active).map(e=>e.id));
    const result = [];
    ...
    state.employees.filter(e=>e.active).forEach(emp => {
      ...
      const hadDayOff = employeeItems.some(item => item.status === "off");
      ...
      if(workItems.length){
        ...
        // ВАЖНО: выходной прошлой недели не повторяем.
        // Если сотрудник был выходной в этот день прошлой недели,
        // на новой неделе возвращаем его в обычную рабочую смену.
        result.push({
          employeeId: emp.id,
      ...
      // Если сотрудник вообще не был назначен в этот день прошлой недели,
      // оставляем его неназначенным. Это сохраняет логику новых сотрудников.
    });
...
function repairUntouchedCopiedWeekV171(weekKey, copyFromKey){
  ...
  if(week.cleanWeekVersion === "17.1") return false;
...
function createNextWeek(){
  ...
  if(!state.weeks[key]){
    ...
    repairUntouchedCopiedWeekV171(key,sourceKey);
  }
