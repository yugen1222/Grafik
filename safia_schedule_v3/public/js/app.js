...
function saveState(options = {}){
  ...
  if(options.immediate){
    ...
    firebaseSaveTimer = setTimeout(write, 180);
  }
}
...
function startFirebaseSync(){
  ...
  databaseRef.on("value", snapshot => {
    ...
    if(!data || !data.state){
      ...
      setSyncStatusV112("Онлайн · создана база", "ok");
      return;
    }

    firebaseStarted = true;
    if(data.updatedBy === window.CLIENT_ID){
      setSyncStatusV112("Сохранено", "ok");
      return;
    }

    isRemoteUpdate = true;
    const localBefore = migrateState(JSON.parse(JSON.stringify(state))) || state;
    const remoteState = migrateState(data.state) || state;

    remoteState.employees = mergeEmployeeLists(
      remoteState.employees,
      localBefore.employees
    );

    // Штатка имеет собственную версию. Более новая штатка никогда не
    // заменяется старой или пустой копией с другого устройства.
    const localStaffingVersion = Number(localBefore.staffingVersion || 0);
    const remoteStaffingVersion = Number(remoteState.staffingVersion || data.staffingVersion || 0);
    let mustPushMergedState = false;

    if(localStaffingVersion > remoteStaffingVersion){
      Object.keys(localBefore.weeks || {}).forEach(weekKey => {
        if(!remoteState.weeks[weekKey]) return;
        remoteState.weeks[weekKey].staffing = JSON.parse(JSON.stringify(
          localBefore.weeks[weekKey].staffing || remoteState.weeks[weekKey].staffing || {}
        ));
      });
      remoteState.staffingVersion = localStaffingVersion;
      mustPushMergedState = true;
    }
    ...
    render({ save:false });
    setSyncStatusV112("Онлайн · данные синхронизированы", "ok");

    if(
      mustPushMergedState ||
      state.employees.length !== (data.state.employees || []).length
    ){
      saveState({ immediate:true });
    }
    ...
    setSyncStatusV112("Нет связи · работаем из памяти телефона", "error");
...
setSyncStatusV112("Загружено из памяти устройства", "saving");
render({ save:false });
...
// ================= SAFIA SCHEDULE V12: KPI МЕНЕДЖЕРОВ В ПРОЦЕНТАХ =================
